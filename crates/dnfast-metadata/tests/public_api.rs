...
use dnfast_metadata::{
    MAX_PACKAGES, MAX_PRIMARY_COMPRESSED_BYTES, MAX_PRIMARY_OPEN_BYTES, MetadataError, Package,
    PrimaryRecord, decode_primary, parse_primary, parse_repomd, search, verify_compressed,
};
...
fn public_types_and_constants_remain_consumer_usable() {
    ...
    assert_eq!(
        ...
        (536_870_912, 1_073_741_824, 2_000_000)
    );
...
fn repomd_parser_preserves_fields_and_error_observables() {
    let xml = format!(
        r#"<repomd xmlns="http://linux.duke.edu/metadata/repo"><data type="primary"><checksum type="sha256">{CHECKSUM}</checksum><open-checksum type="sha256">{CHECKSUM}</open-checksum><location href="repodata/primary.xml"/><size>1</size><open-size>2</open-size></data></repomd>"#
    );
    ...
    assert_eq!(
        ...
        PrimaryRecord {
            ...
            size: 1,
            open_checksum: CHECKSUM.into(),
            open_size: 2
        }
    );
    assert_eq!(
        parse_repomd(b"<wrong/>"),
        Err(MetadataError::Xml("element outside repomd root".into()))
    );
    assert_eq!(
        parse_repomd(xml.replace("repodata/primary.xml", "../x").as_bytes()),
        Err(MetadataError::UnsafeLocation("../x".into()))
    );
    assert_eq!(
        parse_repomd(xml.replace("<size>1</size>", "<size>x</size>").as_bytes()),
        Err(MetadataError::InvalidNumber("x".into()))
    );
    assert_eq!(
        parse_repomd(xml.replace(CHECKSUM, "bad").as_bytes()),
        Err(MetadataError::UnsupportedChecksum("bad".into()))
    );
    assert_eq!(
        parse_repomd(b"<repomd xmlns=\"http://linux.duke.edu/metadata/repo\"></repomd>"),
...
fn primary_parser_and_search_preserve_order_and_borrowing() {
    ...
    assert_eq!(
        search(&packages, " ALPHA "),
        vec![&packages[0], &packages[1]]
    );
    assert_eq!(search(&packages, "needle"), vec![&packages[0]]);
    assert!(search(&packages, "  ").is_empty());
    assert_eq!(
        parse_primary(
            b"<metadata xmlns=\"http://linux.duke.edu/metadata/common\" packages=\"1\"></metadata>"
                .as_slice()
        ),
        Err(MetadataError::Xml(
            "primary package count mismatch: declared 1, parsed 0".into()
        ))
    );
}
...
fn compression_functions_preserve_success_and_failure_precedence() {
    ...
    wrong_size.size = 4;
    assert_eq!(
        decode_primary(bytes, &wrong_size),
    ...
    wrong_hash.checksum = CHECKSUM.into();
    assert_eq!(
        verify_compressed(bytes, &wrong_hash),
    ...
    assert!(matches!(
        decode_primary(malformed, &record(malformed, "repodata/primary.xml")),
...
fn every_error_variant_retains_debug_based_display_and_error_trait() {
    let errors = [
        MetadataError::Xml("x".into()),
        MetadataError::MissingPrimary,
        MetadataError::InvalidNumber("x".into()),
        MetadataError::UnsafeLocation("x".into()),
        MetadataError::UnsupportedChecksum("x".into()),
        MetadataError::ChecksumMismatch,
        MetadataError::SizeMismatch {
            expected: 1,
            actual: 2,
        },
        MetadataError::UnsupportedCompression("x".into()),
        MetadataError::Io("x".into()),
    ];
    for error in errors {

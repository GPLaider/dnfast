...
fn reasons_derive_only_from_actual_post_run_instances() {
    ...
    let proposed = BTreeMap::from([
        (planned("new"), PackageReason::Dependency),
        (planned("missing"), PackageReason::Dependency),
    ]);
    let reconciled = reconcile_reasons(
        &before,
        &after,
        &proposed,
        &BTreeSet::new(),
    ...
    assert_eq!(
        reconciled[0].decision,
        ReasonDecision::Record(PackageReason::Dependency)
    );
    assert_eq!(
        reconciled[1].decision,
...
fn external_protected_and_installonly_are_always_kept() {
    ...
    let proposed = BTreeMap::from([
        (planned("external"), PackageReason::Unknown),
        (planned("kernel"), PackageReason::Dependency),
    ]);
    let results = reconcile_reasons(
        &before,
        &after,
        &proposed,
        &BTreeSet::new(),
        &BTreeSet::from(["kernel".into()]),
    );
    assert!(
        results
            .iter()
            .all(|record| matches!(record.decision, ReasonDecision::Keep(_)))
    );
}
...
fn one_planned_nevra_is_consumed_by_only_one_post_run_instance() {
    ...
    assert_eq!(
        results
            .iter()
            .filter(|record| record.decision == ReasonDecision::Record(PackageReason::Dependency))
            .count(),
        1
    );
    assert_eq!(
        results
            .iter()
            .filter(|record| record.decision == ReasonDecision::Keep(PackageReason::External))
            .count(),
        1
    );
}
...
fn planned(name: &str) -> PlannedIdentity {
    PlannedIdentity {
        package_name: name.into(),
        target_evra: Evra::new(0, "1", "1", Architecture::Aarch64),
    }
}

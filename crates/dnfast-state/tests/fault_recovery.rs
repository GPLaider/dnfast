...
fn every_durable_publish_boundary_failure_has_conservative_authority() {
    for point in [
        ...
        let store =
            JournalStore::with_faults(&temporary.path().join("transactions"), faults.clone())
                .unwrap();
...
fn failed_sequence_zero_publication_cleans_uuid_and_retry_succeeds() {
    for point in [
        ...
        assert_eq!(
            store.create(&id, DIGEST).unwrap().entries().unwrap().len(),
            1
        );
    }
    let temporary = tempfile::tempdir().unwrap();
...
fn actual_os_enospc_source_preserves_prior_sequence_on_write_abort() {
    ...
    let faults = Arc::new(FaultPlan::none());
    let store =
        JournalStore::with_faults(&temporary.path().join("transactions"), faults.clone()).unwrap();
    let id = TransactionId::parse(ID).unwrap();
    let journal = store.create(&id, DIGEST).unwrap();
...
fn bounded_journal_filesystem_enospc_keeps_prepared_authoritative() {
    ...
    let status = std::process::Command::new("unshare")
        ...
        .arg("--exact")
        .arg("journal_enospc_child")
        .arg("--nocapture")
        .env("DNFAST_ENOSPC_MOUNT", temporary.path())
        .status()
        .unwrap();
...
fn journal_enospc_child() {
    let Ok(path) = std::env::var("DNFAST_ENOSPC_MOUNT") else {
        return;
    };

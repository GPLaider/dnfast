    ...
    pub fn parse(value: &str) -> Result<Self, StateError> {
            ...
            || !matches!(bytes.get(19), Some(b'8' | b'9' | b'a' | b'b'))
        {
            return Err(StateError::Corrupt(
                "transaction id is not canonical UUIDv7".into(),
            ));
        }
        Ok(Self(value.into()))
    }
...
pub enum RecoveryAction {
    CleanupRevalidateAndReapprove,
    ReconcileOnly,
    Terminal(ReconcileResult),
}

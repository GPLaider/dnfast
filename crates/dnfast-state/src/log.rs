...
pub(crate) fn append(directory: &OwnedFd, bytes: &[u8]) -> Result<LogAppend, StateError> {
    ...
    let current =
        u64::try_from(stat.st_size).map_err(|_| StateError::Corrupt("negative log size".into()))?;
    if current > MAX_LOG_BYTES {
        return Err(StateError::Corrupt("event log exceeds limit".into()));
    }
    ...
    if current > payload_end {
        file.set_len(payload_end).map_err(io)?;
    }
    file.seek(SeekFrom::Start(current.min(payload_end)))
        .map_err(io)?;
    let take = usize::try_from(payload_end.saturating_sub(current.min(payload_end)))
        .map_err(|_| StateError::Limit("event log"))?;
    file.write_all(&bytes[..take.min(bytes.len())])
        .map_err(io)?;

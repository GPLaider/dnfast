...
pub(super) fn current_snapshot(proposal: &CanonicalSolverPlan) -> Result<PlanningSnapshot, PreparationError> {
    ...
    if snapshot.integrity_for_repositories(&selected).map_err(snapshot_error)? != proposal.proposal().integrity() {
        return Err(PreparationError::SnapshotMismatch);
    }
    Ok(snapshot)
}

pub(super) fn revalidate_snapshot_and_inventory(proposal: &CanonicalSolverPlan) -> Result<(), PreparationError> {
...
pub(super) fn selected_ids(proposal: &CanonicalSolverPlan) -> Vec<String> {
    proposal.proposal().selected_repositories().iter().map(|binding| binding.id().to_owned()).collect()
}

pub(super) fn selected_repositories<'a>(snapshot: &'a PlanningSnapshot, proposal: &CanonicalSolverPlan) -> Result<Vec<&'a PlanningRepository>, PreparationError> {
    let mut selected = proposal.proposal().selected_repositories().iter().map(|binding| {
    ...
    if selected.windows(2).any(|pair| pair[0].id == pair[1].id) {
        return Err(PreparationError::Snapshot("duplicate selected repository".into()));
    }
    Ok(selected)
}

fn current_inventory(architecture: dnfast_core::Architecture) -> Result<InstalledInventory, PreparationError> {
    let mut context = NativeContext::open(architecture, || false).map_err(native)?;
    context.add_installed_rpmdb("/").map_err(native)?;
    context.read_installed_inventory().map_err(|error| PreparationError::Native(error.to_string()))
}

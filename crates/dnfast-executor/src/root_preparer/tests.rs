...
use super::prepared_generation::{InputDraft, PREPARING_PREFIX, Publication, metadata_digest, payload_bytes, trust_digest};
use crate::input_model::InputManifest;

pub(super) fn draft(root: &std::path::Path) -> InputDraft {
    let parent = open(root, OFlags::RDONLY | OFlags::DIRECTORY | OFlags::CLOEXEC | OFlags::NOFOLLOW, Mode::empty()).expect("open test root");
    InputDraft::create_under(parent, root.to_str().expect("UTF-8 test root")).expect("create test draft")
}
...
fn payload_digest_tamper_fails_before_a_generation_is_publishable() {
    ...
    let payload = PlanningBytes { sha256: "a".repeat(64), size: 4, base64: "Z29vZA==".into() };
    ...
    drop(draft);
    let names = fs::read_dir(root.path()).expect("root listing").map(|entry| entry.expect("entry").file_name()).collect::<Vec<_>>();
    assert!(names.iter().all(|name| !name.to_string_lossy().starts_with(PREPARING_PREFIX)));
}
...
fn zstd_snapshot_metadata_materializes_xml_for_the_native_solver_boundary() {
    ...
    let materialized = draft.write_repository(&repository, 0).expect("materialized repository");
    let mut primary_source = draft.open(&materialized.input.primary).expect("source primary");
    let mut filelists_source = draft.open(&materialized.input.filelists).expect("source filelists");
    let mut primary = draft.open(&materialized.native_primary).expect("native primary");
    let mut filelists = draft.open(&materialized.native_filelists).expect("native filelists");
    let mut primary_zstd = Vec::new();
    let mut filelists_zstd = Vec::new();
    let mut primary_xml = Vec::new();
    let mut filelists_xml = Vec::new();
    primary_source.read_to_end(&mut primary_zstd).expect("read source primary");
    filelists_source.read_to_end(&mut filelists_zstd).expect("read source filelists");
    primary.read_to_end(&mut primary_xml).expect("read native primary");
    filelists.read_to_end(&mut filelists_xml).expect("read native filelists");

    // Then: the local inputs are XML that the same rpm-md parsers used to bind the solver can consume.
    assert_eq!(primary_zstd, payload_bytes("primary", &repository.primary).expect("trusted primary"));
    assert_eq!(filelists_zstd, payload_bytes("filelists", &repository.filelists).expect("trusted filelists"));
    ...
    assert!(!dnfast_metadata::parse_primary_records(primary_xml.as_slice()).expect("native primary XML").is_empty());
    assert!(!dnfast_metadata::parse_filelists(filelists_xml.as_slice()).expect("native filelists XML").is_empty());
}
...
fn durable_generation_omits_native_xml_and_redecodes_raw_compressed_metadata() {
    ...
    let metadata_sha256 = metadata_digest(std::slice::from_ref(&materialized.input)).expect("metadata digest");
    let trust_sha256 = trust_digest(std::slice::from_ref(&materialized.input)).expect("trust digest");

    // When: preparation discards its temporary native XML before publication.
    draft.discard_native_metadata(std::slice::from_ref(&materialized)).expect("discard native metadata");
    draft.write_manifest(&InputManifest {
        schema_version: 3,
        policy,
        metadata_sha256,
        trust_sha256,
    ...
    let published = openat2(&draft.parent, &digest, OFlags::RDONLY | OFlags::DIRECTORY | OFlags::CLOEXEC | OFlags::NOFOLLOW,
        Mode::empty(), ResolveFlags::BENEATH | ResolveFlags::NO_SYMLINKS | ResolveFlags::NO_MAGICLINKS).expect("published generation");

    // Then: only raw compressed records are durable, and staging's decode boundary can recover XML.
    assert_eq!(openat(&published, "repo-0-native-primary.xml", OFlags::RDONLY | OFlags::CLOEXEC | OFlags::NOFOLLOW, Mode::empty()).expect_err("native primary absent"), rustix::io::Errno::NOENT);
    assert_eq!(openat(&published, "repo-0-native-filelists.xml", OFlags::RDONLY | OFlags::CLOEXEC | OFlags::NOFOLLOW, Mode::empty()).expect_err("native filelists absent"), rustix::io::Errno::NOENT);
    let mut repomd = Vec::new();
    let mut primary = Vec::new();
    let mut filelists = Vec::new();
    File::from(openat(&published, &repomd_name, OFlags::RDONLY | OFlags::CLOEXEC | OFlags::NOFOLLOW, Mode::empty()).expect("repomd")).read_to_end(&mut repomd).expect("read repomd");
    File::from(openat(&published, &primary_name, OFlags::RDONLY | OFlags::CLOEXEC | OFlags::NOFOLLOW, Mode::empty()).expect("primary")).read_to_end(&mut primary).expect("read primary");
    File::from(openat(&published, &filelists_name, OFlags::RDONLY | OFlags::CLOEXEC | OFlags::NOFOLLOW, Mode::empty()).expect("filelists")).read_to_end(&mut filelists).expect("read filelists");
    ...
    assert!(!dnfast_metadata::decode_primary(&primary, &records.primary).expect("staged primary XML").is_empty());
    assert!(!dnfast_metadata::decode_record(&filelists, &records.filelists).expect("staged filelists XML").is_empty());
}
...
fn primary_materialization_failure_identifies_the_metadata_role() {
    ...
    assert!(matches!(result, Err(PreparationError::Snapshot(message)) if message.starts_with("primary rpm-md materialization failed:")));
}
...
pub(super) fn zstd_repository() -> PlanningRepository {
    ...
    let solver_inputs = dnfast_metadata::parse_primary_records(
        dnfast_metadata::decode_primary(primary.as_slice(), &records.primary).expect("primary XML").as_slice(),
    ).expect("primary records");
    let filelist_inputs = dnfast_metadata::parse_filelists_record(filelists.as_slice(), &records.filelists).expect("filelists records");
...
pub(super) fn planning_bytes(bytes: &[u8]) -> PlanningBytes {
    PlanningBytes { sha256: format!("{:x}", Sha256::digest(bytes)), size: u64::try_from(bytes.len()).expect("fixture length"), base64: base64::engine::general_purpose::STANDARD.encode(bytes) }
}

pub(super) fn frame_digest(digest: &mut Sha256, name: &str, bytes: &[u8]) {
    digest.update(u64::try_from(name.len()).expect("name length").to_be_bytes());
    digest.update(name.as_bytes());
    digest.update(u64::try_from(bytes.len()).expect("byte length").to_be_bytes());
    digest.update(bytes);
}

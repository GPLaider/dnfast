...
use rustix::{
    fs::{AtFlags, Mode, OFlags, StatxFlags, fstat, mkdirat, open, openat2, statx, unlinkat},
    mount::{MountFlags, MountPropagationFlags, mount_bind, mount_change, mount_remount, unmount},
    process::{chdir, chroot, fchdir},
    thread::UnshareFlags,
};
...
impl MountRoot {
    ...
    pub fn restore_namespace_root(&mut self) -> Result<(), ExecutorError> {
        ...
        let host_root = self.host_root.take().ok_or_else(|| {
            ExecutorError::Mount("missing pre-chroot root fd for post-transaction restore".into())
        })?;
    ...
    pub fn verify_unchanged(&self) -> Result<(), ExecutorError> {
        let current = if self.entered_root {
            open(
                "/",
            ...
            open_mount_root(&self.parent, &self.name)?
        };
        if mount_identity(&current)? == mount_identity(&self.root)? {
            Ok(())
        } else {
            Err(ExecutorError::Mount("root bind mount changed".into()))
        }
    }

    #[cfg(feature = "test-fixtures")]
    pub fn fixture_replace_bind_mount(&self) -> Result<(), ExecutorError> {
        std::fs::write("/tmp/.dnfast-mount-swap-sentinel", b"fixture")
            .map_err(|error| ExecutorError::Mount(error.to_string()))?;
        unmount(
            &self.target,
    ...
    #[cfg(feature = "test-fixtures")]
    pub fn fixture_replacement_has_sentinel(&self) -> bool {
        self.target.join(".dnfast-mount-swap-sentinel").is_file()
    }

    #[cfg(feature = "test-fixtures")]
    pub fn fixture_replace_current_root(&self) -> Result<(), ExecutorError> {
        std::fs::write("/tmp/.dnfast-current-root-sentinel", b"fixture")
            .map_err(|error| ExecutorError::Mount(error.to_string()))?;
...
fn mount(error: rustix::io::Errno) -> ExecutorError {
    ExecutorError::Mount(error.to_string())
}
...
mod tests {
    ...
    fn recursive_private_bind_replacement_is_rejected() {
        ...
        root.fixture_replace_bind_mount()
            .expect("replace recursive bind root");
        assert!(
            root.allow_writes().is_err(),
            "replacement must fail retained mount identity"
        );

...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    if rustix::process::geteuid().as_raw() != 0 { return Err("mount swap probe requires root".into()); }
...
fn verify_post_approval_swap() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("post_approval_mountinfo={}", std::fs::read_to_string("/proc/self/mountinfo")?.lines()
        .find(|line| line.split_whitespace().nth(4) == Some("/")).ok_or("root mountinfo absent")?);
    eprintln!("mount_probe=post:replace:before");
    root.fixture_replace_current_root()?;
    if !root.fixture_current_root_has_sentinel() { return Err("post-approval mount swap did not replace root".into()); }
    let rejected = root.verify_unchanged().is_err();
    root.cleanup()?;
    staging.cleanup()?;
    std::fs::remove_file("/.dnfast-current-root-sentinel")?;
    if !rejected { return Err("post-approval mount swap was accepted".into()); }
    Ok(())
}

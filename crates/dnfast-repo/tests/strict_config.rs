...
use dnfast_repo::{apply_setopts, parse_before_network, parse_main_config, parse_repo_profile, MainConfig, MetadataExpire};
...
fn defaults_match_frozen_fedora_profile_when_main_is_empty() {
    ...
    assert!(!config.best);
    assert_eq!(config.reposdir, ["/etc/yum.repos.d", "/etc/dnf/repos.d"].map(std::path::PathBuf::from));
    assert_eq!(config.varsdir, ["/etc/dnf/vars", "/etc/yum/vars"].map(std::path::PathBuf::from));
    assert!(config.excludepkgs.is_empty() && config.includepkgs.is_empty());
    assert_eq!(config.protected_packages, ["dnfast", "dnf", "dnf5", "rpm", "glibc", "systemd"]);
    assert_eq!(config.installonlypkgs, ["kernel", "kernel-core", "kernel-modules", "kernel-modules-core", "kernel-modules-extra"]);
}

#[test]
fn supported_values_and_list_reset_follow_layer_order() {
    // Given main values and a repository section.
    let main = parse_main_config(Path::new("dnf.conf"), "[main]\nbest=true\nexcludepkgs=a b\n").unwrap();
    ...
    let result = apply_setopts(profile, &["main.excludepkgs=".into(), "repo.fedora.excludepkgs=z".into()]).unwrap();
...
fn stock_fedora_rpm_repository_type_is_accepted_without_adding_a_source() {
    ...
    let profile = parse_repo_profile(Path::new("fedora.repo"), input, &MainConfig::default()).unwrap();
    ...
    assert_eq!(repository.metalink.as_deref(), Some("https://mirrors.fedoraproject.org/metalink?repo=fedora-$releasever&arch=$basearch"));
...
fn stock_fedora_countme_is_accepted_as_inert_metadata() {
    ...
    let repository = &profile.repositories[0];
    assert!(repository.baseurl.is_empty());
    assert_eq!(repository.metalink.as_deref(), Some("https://mirrors.fedoraproject.org/metalink?repo=fedora-44&arch=aarch64"));
    assert!(repository.mirrorlist.is_none());
}
...
fn countme_only_accepts_stock_binary_values_before_network() {
    ...
    let disabled_profile = parse_repo_profile(Path::new("fedora.repo"), disabled, &MainConfig::default()).unwrap();
    assert_eq!(disabled_profile.repositories[0].baseurl, ["https://example.test/fedora"]);
    for value in ["2", "true", "false", "yes", "https://untrusted.example.invalid", "1\\0authority"] {
        let input = format!("[fedora]\nbaseurl=https://example.test/fedora\ncountme={value}\n");
        let mut network_calls = 0;
        // When the parse-before-network boundary reads the invalid setting.
        let result = parse_before_network(Path::new("fedora.repo"), &input, &MainConfig::default(), |_| network_calls += 1);
...
fn countme_setopt_is_strict_and_inert() {
    ...
    let error = apply_setopts(profile, &["repo.fedora.countme=https://untrusted.example.invalid".to_owned()]).unwrap_err();
...
fn stock_fedora_metadata_expire_duration_units_are_converted_to_seconds() {
    ...
    let fedora_profile = parse_repo_profile(Path::new("fedora.repo"), fedora, &MainConfig::default()).unwrap();
    let updates_profile = parse_repo_profile(Path::new("fedora-updates.repo"), updates, &MainConfig::default()).unwrap();
    let rawhide_profile = parse_repo_profile(Path::new("fedora-rawhide.repo"), rawhide, &MainConfig::default()).unwrap();
    // Then durations are represented as their exact number of seconds.
    assert_eq!(fedora_profile.repositories[0].metadata_expire, MetadataExpire::AfterSeconds(604_800));
    assert_eq!(updates_profile.repositories[0].metadata_expire, MetadataExpire::AfterSeconds(21_600));
    assert_eq!(rawhide_profile.repositories[0].metadata_expire, MetadataExpire::AfterSeconds(1_209_600));
}
...
fn metadata_expire_accepts_libdnf_hex_integers_with_optional_units() {
    ...
    let seconds_profile = parse_repo_profile(Path::new("fedora.repo"), seconds, &MainConfig::default()).unwrap();
    let hours_profile = parse_repo_profile(Path::new("fedora.repo"), hours, &MainConfig::default()).unwrap();
    let uppercase_profile = parse_repo_profile(Path::new("fedora.repo"), uppercase, &MainConfig::default()).unwrap();
    // Then each value has the exact typed seconds value and cannot alter source authority.
    assert_eq!(seconds_profile.repositories[0].metadata_expire, MetadataExpire::AfterSeconds(15));
    assert_eq!(hours_profile.repositories[0].metadata_expire, MetadataExpire::AfterSeconds(54_000));
    assert_eq!(uppercase_profile.repositories[0].metadata_expire, MetadataExpire::AfterSeconds(54_000));
}
...
fn metadata_expire_uses_the_same_duration_parser_for_cli_overrides() {
    ...
    assert_eq!(profile.repositories[0].metadata_expire, MetadataExpire::AfterSeconds(5_400));
}
...
fn metadata_expire_uses_the_same_hex_duration_parser_for_cli_overrides() {
    ...
    assert_eq!(profile.repositories[0].metadata_expire, MetadataExpire::AfterSeconds(54_000));
}
...
fn metadata_expire_preserves_decimal_fractional_second_semantics() {
    ...
    let profile = parse_repo_profile(Path::new("fedora.repo"), input, &MainConfig::default()).unwrap();
    assert_eq!(profile.repositories[0].metadata_expire, MetadataExpire::AfterSeconds(0));
    ...
    assert_eq!(profile.repositories[0].metadata_expire, MetadataExpire::AfterSeconds(1_800));
}
...
fn metadata_expire_rejects_non_duration_and_overflow_before_network() {
    ...
    for value in [
        "1w", "1h1m", "https://untrusted.example.invalid", "18446744073709551615d", "6h\0authority",
        "1e3", "0x", "0xG", "0xF.1", "0x1p2", "0xFh1", "0xFh://untrusted.example.invalid",
        "0x10000000000000000", "0x1000000000000000h",
    ] {
...
fn metadata_expire_distinguishes_malformed_hex_from_hex_overflow() {
    ...
    // When the strict mutation parser reads each value.
    let malformed_error = parse_repo_profile(Path::new("fedora.repo"), malformed, &MainConfig::default()).unwrap_err();
    let overflow_error = parse_repo_profile(Path::new("fedora.repo"), overflow, &MainConfig::default()).unwrap_err();
    // Then malformed syntax and arithmetic overflow keep distinct diagnostics.
    assert_eq!(malformed_error.to_string(), "fedora.repo:3: invalid metadata_expire");
    assert_eq!(overflow_error.to_string(), "fedora.repo:3: metadata_expire exceeds u64 seconds");
}
...
fn metadata_expire_models_dnf_never_without_colliding_with_seconds() {
    ...
    for value in ["-1", "never"] {
        ...
        assert_eq!(profile.repositories[0].metadata_expire, MetadataExpire::Never);
    }
}
...
fn documented_rpm_md_type_aliases_are_accepted_without_changing_sources() {
    ...
    for alias in ["rpm", "rpm-md", "repomd", "rpmmd", "yum", "YUM"] {
        ...
        assert_eq!(profile.repositories[0].baseurl, ["https://example.test/fedora"]);
...
fn repository_type_rejects_non_rpm_md_values_without_accepting_new_sources() {
    ...
    for value in ["deb", "RPM", "rpm-MD", "Yum"] {
        ...
        // When the strict mutation parser reads it.
        let error = parse_repo_profile(Path::new("foreign.repo"), &input, &MainConfig::default()).unwrap_err();
        // Then the typed configuration boundary rejects each non-documented spelling.
        assert_eq!(error.to_string(), "foreign.repo:2: unsupported repository type");
    }
}
...
fn repository_type_cannot_grant_url_authority() {
    ...
    let error = parse_before_network(Path::new("foreign.repo"), input, &MainConfig::default(), |_| network_calls += 1).unwrap_err();
...
fn forbidden_and_unknown_settings_fail_closed_without_leaking_values() {
    ...
    for input in ["[main]\npluginconfpath=/secret/token\n", "[main]\nunknown=value\n"] {
        ...
        let error = parse_main_config(Path::new("dnf.conf"), input).unwrap_err().to_string();
...
fn setopt_supports_every_frozen_main_key_and_repo_trust_reset() {
    ...
    let options = ["main.reposdir=", "main.varsdir=/safe", "main.install_weak_deps=false", "main.best=true", "main.excludepkgs=x", "main.includepkgs=y", "main.protected_packages=", "main.installonlypkgs=custom", "main.installonly_limit=0", "repo.a.gpgkey=", "repo.a.dnfast_allowed_fingerprints="].map(str::to_owned);
    ...
    assert_eq!(result.main.varsdir, [std::path::PathBuf::from("/etc/dnf/vars"), "/etc/yum/vars".into(), "/safe".into()]);
...
fn repo_trust_defaults_are_explicit_and_credentials_fail_before_network() {
    ...
    let result = parse_repo_profile(Path::new("x"), "[a]\nbaseurl=x\n", &MainConfig::default()).unwrap();
    let repo = &result.repositories[0];
    assert!(repo.sslverify && repo.gpgcheck && repo.pkg_gpgcheck && !repo.repo_gpgcheck);
    assert!(repo.enabled);
    assert_eq!((repo.priority, repo.cost, repo.metadata_expire), (99, 1000, MetadataExpire::AfterSeconds(172_800)));
    assert!(!repo.skip_if_unavailable);
    assert!(repo.name.is_none() && repo.metalink.is_none() && repo.mirrorlist.is_none() && repo.proxy.is_none());
    ...
    let source_empty = parse_repo_profile(Path::new("x"), "[a]\nenabled=false\n", &MainConfig::default()).unwrap();
...
fn repository_and_numeric_limits_accept_boundary_and_reject_plus_one() {
    ...
    for setting in ["cost=4294967296", "metadata_expire=18446744073709551616", "priority=65536"] {
        assert!(parse_repo_profile(Path::new("x"), &format!("[a]\nbaseurl=x\n{setting}\n"), &MainConfig::default()).is_err());
    }
    let repositories = (0..1024).map(|index| format!("[r{index}]\nbaseurl=x\n")).collect::<String>();
...
fn every_repo_key_parses_defaults_repo_values_and_cli_precedence() {
    ...
    assert!(repo.enabled && repo.skip_if_unavailable && repo.sslverify && repo.gpgcheck && repo.pkg_gpgcheck && !repo.repo_gpgcheck);
    assert_eq!((repo.priority, repo.cost, repo.metadata_expire), (7, 8, MetadataExpire::AfterSeconds(9)));
    ...
    let options = ["repo.a.name=CLI", "repo.a.type=rpm-md", "repo.a.enabled=false", "repo.a.baseurl=", "repo.a.metalink=", "repo.a.mirrorlist=", "repo.a.priority=10", "repo.a.cost=11", "repo.a.skip_if_unavailable=false", "repo.a.metadata_expire=12", "repo.a.sslverify=true", "repo.a.proxy=_none_", "repo.a.excludepkgs=", "repo.a.excludepkgs=cli", "repo.a.includepkgs=", "repo.a.gpgcheck=true", "repo.a.pkg_gpgcheck=true", "repo.a.repo_gpgcheck=false", "repo.a.gpgkey=", "repo.a.dnfast_allowed_fingerprints="].map(str::to_owned);
    ...
    assert!(repo.baseurl.is_empty() && repo.metalink.is_none() && repo.mirrorlist.is_none() && repo.proxy.is_none());
    assert_eq!((repo.priority, repo.cost, repo.metadata_expire), (10, 11, MetadataExpire::AfterSeconds(12)));
    assert_eq!(repo.excludepkgs, ["cli"]);
    assert!(repo.includepkgs.is_empty() && repo.gpgkey.is_empty() && repo.allowed_fingerprints.is_empty());
}

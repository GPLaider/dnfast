...
use crate::{
    MutationError,
    anchored_fs::{read_root_file, read_system_gpg_key},
};
...
mod system_key_tests {
    ...
    fn system_gpg_file_uri_and_root_owned_alias_are_accepted_without_losing_per_repo_binding() {
        ...
        fs::set_permissions(&key, fs::Permissions::from_mode(0o644))
            .expect("key mode must be trusted");
        ...
        let normalized = normalize_gpgkey_location(&format!("file://{}", alias.display()))
    ...
    fn system_gpg_rejects_url_outside_user_owned_and_escaping_symlink_before_trust_can_change() {
        ...
        fs::set_permissions(&outside, fs::Permissions::from_mode(0o644))
            .expect("outside mode must be trusted");
        let escaping = directory.join("escape");
        symlink("../dnfast-system-gpg-reject-outside", &escaping)
            .expect("escaping alias must be created");
        let owner = fs::metadata(&directory)
            .expect("fixture metadata must exist")
            .uid();
        ...
        let user_owned = key_bundle_digest_owned(
            "fedora",
            &[key],
            Path::new("/etc/dnfast/keys"),
            0,
        ...
        let escaping_alias = key_bundle_digest_owned(
            "fedora",
            &[escaping],
            Path::new("/etc/dnfast/keys"),
            owner,
...
pub fn normalize_gpgkey_location(value: &str) -> Result<PathBuf, MutationError> {
    ...
    let path = if let Some(suffix) = value.strip_prefix("file:///") {
        if suffix.is_empty() || suffix.starts_with('/') {
            return Err(MutationError::new(label, 0, "gpgkey file URI is invalid"));
        }
        ...
        return Err(MutationError::new(
            label,
            0,
            "gpgkey is not a local file URI",
        ));
    } else {
        PathBuf::from(value)
    };
            ...
            .any(|part| matches!(part, Component::ParentDir | Component::CurDir))
    {
        return Err(MutationError::new(
            label,
            0,
            "gpgkey is not an absolute local path",
        ));
    }
    Ok(path)
}
...
fn key_bundle_digest_owned(
    repo_id: &str,
    paths: &[PathBuf],
    parent: &Path,
    owner: u32,
    system_directory: Option<&Path>,
) -> Result<KeyBundle, MutationError> {
    let label = Path::new("<gpgkey>");
    if !valid_id(repo_id) {
        return Err(MutationError::new(label, 0, "invalid repository id"));
    }
    if paths.len() > MAX_KEY_FILES {
        return Err(MutationError::new(
            label,
            0,
            "gpgkey file limit exceeds 256",
        ));
    }
    ...
    for path in paths {
        if !seen.insert(path) {
            return Err(MutationError::new(label, 0, "duplicate gpgkey path"));
        }
        let bytes = match validate_lexical_path(label, path, &root, system_directory)? {
            ...
            KeyLocation::System(directory) => {
                read_system_gpg_key(path, directory, MAX_KEY_BYTES, owner)
            }
        }
        .map_err(|_| MutationError::new(path, 0, "untrusted or oversized gpgkey"))?;
        let encoded = path
            .to_str()
            .ok_or_else(|| MutationError::new(label, 0, "gpgkey path is not UTF-8"))?
            .as_bytes();
        digest.update(
            u64::try_from(encoded.len())
                .map_err(|_| MutationError::new(label, 0, "gpgkey path too long"))?
                .to_be_bytes(),
        );
        digest.update(encoded);
        digest.update(
            u64::try_from(bytes.len())
                .map_err(|_| MutationError::new(label, 0, "gpgkey file too large"))?
                .to_be_bytes(),
        );
        digest.update(&bytes);
...
fn validate_lexical_path<'a>(
    label: &Path,
    path: &Path,
    root: &Path,
    system_directory: Option<&'a Path>,
) -> Result<KeyLocation<'a>, MutationError> {
    if path.to_str().is_none() {
        return Err(MutationError::new(label, 0, "gpgkey path is not UTF-8"));
    }
    if !path.is_absolute()
        || path
            .components()
            .any(|part| matches!(part, Component::ParentDir | Component::CurDir))
    {
        return Err(MutationError::new(
            label,
            0,
            "gpgkey path is not absolute and canonical",
        ));
    }
    if path.starts_with(root) && path != root {
        return Ok(KeyLocation::Repository);
    }
    ...
    Err(MutationError::new(
        label,
        0,
        "gpgkey path is outside trusted key directories",
    ))
}
...
mod tests {
    ...
    fn digest_bytes_order_size_and_count_boundaries() {
        ...
        assert_ne!(
            ...
            key_bundle_digest_owned(
                "repo",
                &paths.iter().rev().cloned().collect::<Vec<_>>(),
                &parent,
                owner,
                None
            )
        ...
        assert!(
            key_bundle_digest_owned(
                "repo",
                &[paths.clone(), vec![paths[0].clone()]].concat(),
                &parent,
                owner,
                None
            )

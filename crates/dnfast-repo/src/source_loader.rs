use std::{collections::BTreeSet, os::unix::fs::MetadataExt, path::{Path, PathBuf}, process::Command};

use crate::{anchored_fs, key_bundle_digest, parse_main_config, parse_repo_profile, profile::expand_variables, MutationError, MutationProfile, Variables};
...
pub fn load_mutation_profile_from(main_path: &Path) -> Result<MutationProfile, MutationError> {
    let owner = std::fs::symlink_metadata(main_path).map_err(|_| MutationError::new(main_path, 0, "cannot inspect main configuration"))?.uid();
    load_mutation_profile_owned(main_path, owner, None)
}

fn load_mutation_profile_owned(main_path: &Path, owner: u32, system_variables: Option<SystemVariables>) -> Result<MutationProfile, MutationError> {
    ...
    let mut output = MutationProfile { main: main.clone(), repositories: Vec::new(), variables: Variables::default() };
    let mut ids = BTreeSet::new();
    for path in paths {
        let text = read_root_file(&path, owner)?;
        let parsed = parse_repo_profile(&path, &text, &main)?;
        for repo in parsed.repositories {
            if !ids.insert(repo.id.clone()) { return Err(MutationError::new(&path, 0, "duplicate repository id across files")); }
            if ids.len() > 1024 { return Err(MutationError::new(&path, 0, "repository limit exceeds 1024")); }
...
fn trusted_rpm_evaluate(expression: &str) -> Result<String, MutationError> {
    ...
    let output = Command::new(executable)
        ...
        .output()
        .map_err(|_| MutationError::new(executable, 0, "cannot evaluate trusted RPM system variable"))?;
    if !output.status.success() {
        return Err(MutationError::new(executable, 0, "trusted RPM system variable evaluation failed"));
    }
    String::from_utf8(output.stdout)
        .map(|value| value.trim_end_matches(['\n', '\r']).to_owned())
        .map_err(|_| MutationError::new(executable, 0, "trusted RPM system variable is not valid UTF-8"))
}

fn system_variables_from_values(releasever: &str, basearch: &str) -> Result<SystemVariables, MutationError> {
    if releasever.is_empty() || releasever.len() > 32 || !releasever.bytes().all(|byte| byte.is_ascii_digit()) {
        return Err(MutationError::new(Path::new("<system-rpm>"), 0, "invalid Fedora releasever"));
    }
    if !matches!(basearch, "aarch64" | "x86_64") {
        return Err(MutationError::new(Path::new("<system-rpm>"), 0, "unsupported RPM base architecture"));
    }
    Ok(SystemVariables { releasever: releasever.into(), basearch: basearch.into() })
}

#[cfg(test)]
fn load_system_mutation_profile_from_with_values(main_path: &Path, releasever: &str, basearch: &str) -> Result<MutationProfile, MutationError> {
    let owner = std::fs::symlink_metadata(main_path).map_err(|_| MutationError::new(main_path, 0, "cannot inspect main configuration"))?.uid();
    load_mutation_profile_owned(main_path, owner, Some(system_variables_from_values(releasever, basearch)?))
}

fn repository_paths(directories: &[PathBuf], prior_count: usize, owner: u32) -> Result<Vec<PathBuf>, MutationError> {
    ...
    for directory in directories {
        ...
        for entry in std::fs::read_dir(directory).map_err(|_| MutationError::new(directory, 0, "cannot read repository directory"))? {
            let path = entry.map_err(|_| MutationError::new(directory, 0, "cannot read repository entry"))?.path();
            let metadata = std::fs::symlink_metadata(&path).map_err(|_| MutationError::new(&path, 0, "cannot inspect repository entry"))?;
            if path.extension().is_some_and(|extension| extension == "repo") && metadata.is_file() && !metadata.file_type().is_symlink() { paths.push(path); }
        }
    }
    paths.sort_by(|left, right| left.as_os_str().as_encoded_bytes().cmp(right.as_os_str().as_encoded_bytes()));
    if paths.len() + prior_count > MAX_CONFIG_FILES { return Err(MutationError::new(Path::new("<reposdir>"), 0, "configuration file limit exceeds 256")); }
    Ok(paths)
}

fn load_variable_sources(directories: &[PathBuf], owner: u32) -> Result<(Variables, usize), MutationError> {
    let mut count = 0;
    ...
    for directory in directories {
        ...
        let mut paths = std::fs::read_dir(directory).map_err(|_| MutationError::new(directory, 0, "cannot read variables directory"))?
            .map(|entry| entry.map(|value| value.path()).map_err(|_| MutationError::new(directory, 0, "cannot read variable entry")))
            .collect::<Result<Vec<_>, _>>()?;
        paths.sort_by(|left, right| left.as_os_str().as_encoded_bytes().cmp(right.as_os_str().as_encoded_bytes()));
        for path in paths {
            validate_root_metadata(&path, false, owner)?;
            let name = path.file_name().and_then(|value| value.to_str()).ok_or_else(|| MutationError::new(&path, 0, "variable filename is not UTF-8"))?;
            if !name.bytes().all(|byte| byte == b'_' || byte.is_ascii_alphanumeric()) { return Err(MutationError::new(&path, 0, "invalid variable filename")); }
            pairs.push((name.to_owned(), read_root_file(&path, owner)?.trim().to_owned()));
            count += 1;
            if count + 1 > MAX_CONFIG_FILES { return Err(MutationError::new(&path, 0, "configuration file limit exceeds 256")); }
        }
    }
    Ok((Variables::from_pairs(pairs), count))
}

fn validate_root_metadata(path: &Path, directory: bool, owner: u32) -> Result<std::fs::Metadata, MutationError> {
    let metadata = std::fs::symlink_metadata(path).map_err(|_| MutationError::new(path, 0, "cannot inspect root configuration source"))?;
    if metadata.file_type().is_symlink() || metadata.uid() != owner || metadata.mode() & 0o022 != 0 || directory != metadata.is_dir() {
        return Err(MutationError::new(path, 0, "untrusted root configuration source"));
    }
    Ok(metadata)
}
...
fn read_root_file(path: &Path, owner: u32) -> Result<String, MutationError> {
    ...
    String::from_utf8(bytes).map_err(|_| MutationError::new(path, 0, "configuration source is not valid UTF-8"))
}
...
mod tests {
    ...
    impl Fixture {
        fn new() -> Self {
            ...
            let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join(format!(".dnfast-system-vars-{}-{nonce}", std::process::id()));
            fs::create_dir(&root).expect("fixture root must be created");
            fs::set_permissions(&root, fs::Permissions::from_mode(0o755)).expect("fixture root mode must be set");
            Self(root)
        }
        ...
        fn directory(&self, name: &str) -> PathBuf {
            ...
            fs::set_permissions(&path, fs::Permissions::from_mode(0o755)).expect("fixture directory mode must be set");
            path
        }

        fn file(path: &Path, contents: &str) {
            fs::write(path, contents).expect("fixture file must be written");
            fs::set_permissions(path, fs::Permissions::from_mode(0o644)).expect("fixture file mode must be set");
        }
    }
    ...
    fn root_system_profile_expands_stock_fedora_metalink_from_rpm_values() {
        ...
        Fixture::file(&repositories.join("fedora.repo"), "[fedora]\ntype=rpm\nmetalink=https://mirrors.fedoraproject.org/metalink?repo=fedora-$releasever&arch=$basearch\n");
        let main = fixture.0.join("dnf.conf");
        Fixture::file(&main, &format!("[main]\nreposdir={}\nvarsdir={}\n", repositories.display(), variables.display()));
        ...
        assert_eq!(profile.repositories[0].metalink.as_deref(), Some("https://mirrors.fedoraproject.org/metalink?repo=fedora-44&arch=aarch64"));
        assert_eq!(profile.variables.expand("$releasever/$basearch/$arch").expect("system values must remain available"), "44/aarch64/aarch64");
    }
    ...
    fn root_system_profile_accepts_the_stock_fedora_system_gpgkey_alias() {
        ...
        assert_eq!(profile.repositories[0].gpgkey, ["/etc/pki/rpm-gpg/RPM-GPG-KEY-fedora-44-aarch64"]);
    ...
    fn system_variable_values_reject_missing_and_malicious_rpm_output_before_profile_loading() {
        ...
        for (releasever, basearch) in [
            ...
            assert!(result.is_err(), "must reject releasever={releasever:?}, basearch={basearch:?}");
        }
    }
    ...
    fn root_system_profile_rejects_caller_home_variable_before_refresh_can_start() {
        ...
        assert_eq!(error.to_string(), "<variables>:0: unresolved repository variable: HOME");
    }
}

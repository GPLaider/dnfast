...
fn parse(path: &Path, input: &str, id: &str) -> Result<RefreshPolicy, RepoError> {
    ...
    for (index, raw) in input.lines().enumerate() {
        ...
        match key.trim().to_ascii_lowercase().as_str() {
            ...
            "sslverify" if !boolean(path, index + 1, value.trim())? => {
                return Err(parse_error(path, index + 1, "sslverify=false is forbidden"));
            }
            "proxy" if !matches!(value.trim(), "" | "_none_") => {
                return Err(parse_error(
                    path,
                    index + 1,
                    "proxy is unsupported for secure refresh",
                ));
            }
            _ => {}
        }

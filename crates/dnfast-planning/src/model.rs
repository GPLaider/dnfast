...
impl PlanningSnapshot {
    pub(crate) fn new(published_at_unix: u64, payload: PlanningPayload) -> Result<Self, PlanningError> {
        let value = Self { schema_version: SNAPSHOT_SCHEMA_VERSION, published_at_unix, payload };
        value.validate()?;
        Ok(value)
    }

    pub const fn published_at_unix(&self) -> u64 { self.published_at_unix }
    pub fn payload(&self) -> &PlanningPayload { &self.payload }
    ...
    pub fn integrity_for_repositories(&self, selected_repository_ids: &[String]) -> Result<PlanIntegrity, PlanningError> {
        self.validate()?;
        let selected = self.selected_repositories(selected_repository_ids)?;
        let policy = self.payload.policy.solver.canonical_sha256().map_err(domain)?;
        ...
        let bindings = selected.into_iter().map(|repository| {
            let trust = repository.trust.canonical_sha256()?;
            RepositoryBinding::new(repository.id.clone(),
                dnfast_core::Sha256Digest::parse(repository.generation_sha256.clone(), "generation_sha256")?,
                dnfast_core::Sha256Digest::parse(repository.origin.sha256.clone(), "origin_sha256")?, trust)
        }).collect::<Result<Vec<_>, dnfast_core::DomainError>>().map_err(domain)?;
        PlanIntegrity::new([policy.as_str(), trust.as_str(), inventory.as_str(), metadata.as_str(), snapshot.as_str()], bindings).map_err(domain)
    }

    pub(crate) fn from_canonical_bytes(bytes: &[u8]) -> Result<Self, PlanningError> {
        if bytes.len() > MAX_SNAPSHOT_BYTES {
            return Err(PlanningError::UnsafeSnapshot("snapshot exceeds maximum size".into()));
        }
        let value: Self = serde_json::from_slice(bytes).map_err(json)?;
        if value.canonical_bytes()? != bytes {
            return Err(PlanningError::UnsafeSnapshot("snapshot is not canonical JSON".into()));
        }
        value.validate()?;
        Ok(value)
    }
    ...
    fn validate(&self) -> Result<(), PlanningError> {
        ...
        if self.payload.allowed_repositories.is_empty()
            || self.payload.allowed_repositories.windows(2).any(|pair| pair[0].id >= pair[1].id)
            || self.payload.configuration.windows(2).any(|pair| pair[0].id >= pair[1].id)
        {
            return Err(PlanningError::Input("repositories are not canonical".into()));
        }
        self.payload.policy.solver.ensure_supported().map_err(domain)?;
        self.payload.inventory.canonical_sha256().map_err(domain)?;
        for repository in &self.payload.allowed_repositories {
            validate_repository(repository)?;
            let configuration = self.payload.configuration.iter().find(|configuration| configuration.id == repository.id)
                .ok_or_else(|| PlanningError::Input("allowed repository is absent from configuration".into()))?;
            if !configuration.enabled || configuration.priority != repository.priority || configuration.cost != repository.cost
                || configuration.gpgkey != repository.keys.iter().map(|key| key.bundle_path.clone()).collect::<Vec<_>>()
            {
                return Err(PlanningError::Input("allowed repository differs from configuration".into()));
            }
            let mut fingerprints = configuration.allowed_fingerprints.iter().map(|fingerprint| fingerprint.to_ascii_uppercase()).collect::<Vec<_>>();
            fingerprints.sort();
    ...
    fn selected_repositories<'a>(&'a self, selected_repository_ids: &[String]) -> Result<Vec<&'a PlanningRepository>, PlanningError> {
        let mut requested = if selected_repository_ids.is_empty() {
            self.payload.allowed_repositories.iter().map(|repository| repository.id.clone()).collect::<Vec<_>>()
        } else {
            selected_repository_ids.to_vec()
        };
        requested.sort();
        if requested.windows(2).any(|pair| pair[0] == pair[1]) {
            return Err(PlanningError::Input("selected repository identifiers are duplicate".into()));
        }
        requested.into_iter().map(|id| self.payload.allowed_repositories.iter().find(|repository| repository.id == id)
            .ok_or_else(|| PlanningError::Input("selected repository is not root-published and enabled".into())))
            .collect()
    }
}
...
impl PlanningBytes {
    pub(crate) fn from_verified(bytes: &dnfast_cache::VerifiedBytes) -> Self {
        Self { sha256: bytes.sha256().into(), size: bytes.size(), base64: STANDARD.encode(bytes.bytes()) }
    }

    pub(crate) fn decode_verified(&self) -> Result<Vec<u8>, PlanningError> {
        let decoded = self.decode()?;
        if u64::try_from(decoded.len()).map_err(|error| PlanningError::Input(error.to_string()))? != self.size
            || format!("{:x}", Sha256::digest(&decoded)) != self.sha256
        {
            return Err(PlanningError::Input("snapshot payload digest differs".into()));
        }
        Ok(decoded)
    }

    fn decode(&self) -> Result<Vec<u8>, PlanningError> {
        STANDARD.decode(&self.base64).map_err(|_| PlanningError::Input("invalid base64 snapshot payload".into()))
    }
}
...
fn validate_repository(repository: &PlanningRepository) -> Result<(), PlanningError> {
    if repository.id.is_empty()
        ...
        || repository.keys.windows(2).any(|pair| pair[0].bundle_path >= pair[1].bundle_path)
    {
        return Err(PlanningError::Input("repository payload is not canonical".into()));
    }
    repository.materialize_native_xml()?;
    if format!("{:x}", Sha256::digest(repository.origin.repomd_url.as_bytes())) != repository.origin.sha256 {
        return Err(PlanningError::Input("selected origin digest differs".into()));
    }
    ...
    for key in &repository.keys {
        ...
        let certificate = STANDARD.decode(&key.certificate_base64).map_err(|_| PlanningError::Input("key certificate is not base64".into()))?;
        frame(&mut bundle, &key.bundle_path, &certificate)?;
    }
    if format!("{:x}", bundle.finalize()) != repository.trust.key_bundle_sha256().as_str() {
        return Err(PlanningError::Input("key bundle differs from trust policy".into()));
    }
    Ok(())
}

fn domain(error: dnfast_core::DomainError) -> PlanningError { PlanningError::Input(error.to_string()) }
fn json(error: serde_json::Error) -> PlanningError { PlanningError::Input(error.to_string()) }

fn frame(digest: &mut Sha256, name: &str, bytes: &[u8]) -> Result<(), PlanningError> {
    digest.update(u64::try_from(name.len()).map_err(|error| PlanningError::Input(error.to_string()))?.to_be_bytes());
    digest.update(name.as_bytes());
    digest.update(u64::try_from(bytes.len()).map_err(|error| PlanningError::Input(error.to_string()))?.to_be_bytes());
    digest.update(bytes);
    Ok(())
}
...
fn metadata_digest(repositories: &[&PlanningRepository]) -> Result<String, PlanningError> {
    ...
    for repository in repositories {
        ...
        for bytes in [&repository.repomd, &repository.primary, &repository.filelists] {

...
pub use model::{PlanningBytes, PlanningConfiguration, PlanningKey, PlanningOrigin, PlanningPayload, PlanningPolicy, PlanningRepository, PlanningSnapshot};
pub use native_xml::NativeRepositoryXml;
pub use publisher::{PlanningRoots, RootPlanningPublisher, SYSTEM_CACHE_PATH, SYSTEM_PLANNING_PATH, host_rpm_architecture};
...
mod tests {
    ...
    use base64::Engine;
    use dnfast_core::{Architecture, Evra, InstalledInventory, InstalledPackage, RepoPreference, RepoTrustPolicy, SigningSubkeyRule, SolverPolicy};
    use rustix::process::getuid;
    use sha2::{Digest, Sha256};

    use super::{
        PlanningBytes, PlanningConfiguration, PlanningKey, PlanningOrigin, PlanningPayload, PlanningPolicy,
        PlanningRepository, PlanningRoots, PlanningSnapshot, RootPlanningPublisher, host_rpm_architecture,
        fs::TrustedDirectory, snapshot_store::{garbage_collect, publish_snapshot},
    };
    ...
    impl Fixture {
        fn new() -> Self {
            ...
            let path = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join(format!(".planning-test-{}-{nonce}", std::process::id()));
    ...
    impl Drop for Fixture {
        fn drop(&mut self) { fs::remove_dir_all(&self.0).expect("fixture cleanup"); }
    }
    ...
    fn public_reader_returns_the_same_full_payload_twice_and_rejects_mutation() {
        ...
        let metadata = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../fixtures/rpm/generated-build10/repos/main/repodata");
        ...
        let decoded_primary = dnfast_metadata::decode_record(primary.as_slice(), &records.primary).expect("decoded primary");
        let solver_inputs = dnfast_metadata::parse_primary_records(decoded_primary.as_slice()).expect("solver inputs");
        let filelist_inputs = dnfast_metadata::parse_filelists_record(filelists.as_slice(), &records.filelists).expect("filelist inputs");
        ...
        let trust = RepoTrustPolicy::new("main", format!("{:x}", bundle.finalize()), ["A".repeat(40)], SigningSubkeyRule::AuthorizedSubkeys, 7).expect("trust");
        let payload = PlanningPayload {
            policy: PlanningPolicy {
                ...
                included_packages: Vec::new(), installonly_limit: 3,
            },
            ...
            allowed_repositories: vec![PlanningRepository {
                ...
                origin: PlanningOrigin { repomd_url: "https://mirror.example/fedora/repodata/repomd.xml".into(), sha256: format!("{:x}", Sha256::digest("https://mirror.example/fedora/repodata/repomd.xml")) },
        ...
        trust_mutated_payload.allowed_repositories[0].trust = RepoTrustPolicy::new(
            "main", trust_mutated_payload.allowed_repositories[0].trust.key_bundle_sha256().as_str(), ["B".repeat(40)],
        ...
        let trust_mutated = PlanningSnapshot::new(7, trust_mutated_payload).expect("trust-mutated snapshot");
        let mutated_integrity = trust_mutated.integrity_for_repositories(&["main".into()]).expect("trust-mutated integrity");
        assert_ne!(default_selection.metadata_sha256(), mutated_integrity.metadata_sha256());
        ...
        configuration_mutated_payload.configuration[0].includes.push("changed".into());
        let configuration_mutated = PlanningSnapshot::new(7, configuration_mutated_payload).expect("configuration-mutated snapshot");
        assert!(super::publisher::require_same_source_payload(&snapshot, &configuration_mutated).is_err());
        let mut origin_mutated_payload = snapshot.payload().clone();
        origin_mutated_payload.allowed_repositories[0].origin.repomd_url = "https://other.example/repo/repodata/repomd.xml".into();
        origin_mutated_payload.allowed_repositories[0].origin.sha256 = format!("{:x}", Sha256::digest("https://other.example/repo/repodata/repomd.xml"));
        let origin_mutated = PlanningSnapshot::new(7, origin_mutated_payload).expect("origin-mutated snapshot");
        assert!(super::publisher::require_same_source_payload(&snapshot, &origin_mutated).is_err());
        let pointer_mutated = PlanningSnapshot::new(8, snapshot.payload().clone()).expect("pointer-mutated snapshot");
        assert!(super::publisher::require_current_snapshot(&snapshot, &pointer_mutated).is_err());
        assert!(snapshot.integrity_for_repositories(&["disabled".into()]).is_err());
        assert!(snapshot.integrity_for_repositories(&["main".into(), "main".into()]).is_err());
        let roots = PlanningRoots::for_test(&fixture.0);
        let owner = getuid().as_raw();
        let planning = TrustedDirectory::open(roots.planning_root(), owner, true, 0o755).expect("planning root");
        let snapshots = planning.child("snapshots", true, 0o755).expect("snapshot directory");
        ...
        reader.publish_inventory_onto_current(replacement.clone()).expect("inventory-only republish");
        ...
        assert_eq!(republished.payload().allowed_repositories, original_payload.allowed_repositories);
        assert_eq!(republished.payload().allowed_repositories[0].trust, original_payload.allowed_repositories[0].trust);
        assert_eq!(republished.payload().configuration, original_payload.configuration);

        let mut current = digest;
        for published_at in 8..18 {
            let retained = PlanningSnapshot::new(published_at, snapshot.payload().clone()).expect("retained snapshot");
            let retained_bytes = retained.canonical_bytes().expect("retained bytes");
            current = retained.digest().expect("retained digest");
            publish_snapshot(&planning, &snapshots, &current, &retained_bytes).expect("retained publication");
        }
        garbage_collect(&snapshots, &current).expect("bounded collection");
        let retained = fs::read_dir(roots.planning_root().join("snapshots")).expect("snapshot listing").count();
        assert_eq!(retained, 8);
        let snapshot_file = roots.planning_root().join("snapshots").join(current).join("snapshot.json");
    ...
    fn publisher_prepares_and_revalidates_the_trusted_refresh_cache() {
        ...
        publisher.prepare_system_cache_for_verified_refresh().expect("trusted cache root must be created");
        fs::set_permissions(roots.cache_root(), fs::Permissions::from_mode(0o777)).expect("unsafe cache mode");

        assert!(publisher.prepare_system_cache_for_verified_refresh().is_err());
    }
    ...
    fn host_rpm_architecture_ignores_untrusted_process_environment() {
        ...
        if let Ok(marker) = std::env::var(CHILD_MARKER) {
            ...
            assert_eq!(host_rpm_architecture().expect("host architecture"), expected);
        ...
        fs::write(&fake, format!("#!/bin/sh\nprintf shadowed > {}\nprintf 'aarch64\\n'\n", marker.display())).expect("fake rpm");
        fs::set_permissions(&fake, fs::Permissions::from_mode(0o700)).expect("fake rpm mode");
        let status = Command::new(std::env::current_exe().expect("test executable"))
            .args(["--exact", "tests::host_rpm_architecture_ignores_untrusted_process_environment", "--nocapture"])
            .env("PATH", &fixture.0).env("RPM_CONFIGDIR", malicious_configdir)
            .env(CHILD_MARKER, &marker).status().expect("shadow child");
        assert!(status.success());
        assert!(!marker.exists());
    }

    fn bytes(input: &[u8]) -> PlanningBytes {
        PlanningBytes { sha256: format!("{:x}", Sha256::digest(input)), size: u64::try_from(input.len()).expect("length"), base64: base64::engine::general_purpose::STANDARD.encode(input) }
    }
    ...
    fn rpm_architecture(executable: &Path) -> dnfast_core::Architecture {
        match String::from_utf8(Command::new(executable).env_clear().args(["--eval", "%{_arch}"]).output().expect("rpm output").stdout).expect("rpm UTF-8").trim() {

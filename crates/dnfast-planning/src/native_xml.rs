...
impl PlanningRepository {
    pub fn materialize_native_xml(&self) -> Result<NativeRepositoryXml, PlanningError> {
        ...
        let primary = dnfast_metadata::decode_record(&primary, &records.primary)
            .map_err(|error| materialization_error("primary", error))?;
        ...
        if parsed_primary != self.solver_inputs {
            return Err(PlanningError::Input(
                "solver inputs differ from primary metadata".into(),
            ));
        }
        let filelists = self
            .filelists
        ...
        let filelists = dnfast_metadata::decode_record(&filelists, &records.filelists)
            .map_err(|error| materialization_error("filelists", error))?;
        let parsed_filelists = dnfast_metadata::parse_filelists(filelists.as_slice())
            .map_err(|error| materialization_error("filelists", error))?;
        if parsed_filelists != self.filelist_inputs {
            return Err(PlanningError::Input(
                "filelist inputs differ from filelists metadata".into(),
            ));
        }

...
pub(crate) fn parse<T: DeserializeOwned + Serialize>(bytes: &[u8]) -> Result<T, DomainError> {
    inspect(bytes)?;
    let value: T = serde_json::from_slice(bytes).map_err(|error| DomainError::Json(error.to_string()))?;
    if serialize(&value)? != bytes { return Err(DomainError::NonCanonical); }
    Ok(value)
}
...
fn inspect(bytes: &[u8]) -> Result<(), DomainError> {
    if bytes.len() > MAX_DOCUMENT_BYTES { return Err(DomainError::DocumentTooLarge); }
    ...
    for byte in bytes {
        if in_string {
            if escaped { escaped = false; string_bytes += 1; continue; }
            match byte {
                b'\\' => escaped = true,
                b'"' => { in_string = false; string_bytes = 0; }
                _ => { string_bytes += 1; if string_bytes > MAX_STRING_BYTES { return Err(DomainError::StringTooLarge); } }
            }
        } else {
            match byte {
                b'"' => in_string = true,
                b'{' | b'[' => { depth += 1; if depth > MAX_DEPTH { return Err(DomainError::DepthExceeded); } }

...
use crate::{CanonicalDocument, DomainError, PackageReason, Sha256Digest, canonical};
...
impl JournalRecord {
    pub fn new(
        ...
        Ok(Self {
            schema_version: 1,
            plan_sha256: Sha256Digest::parse(plan_sha256, "plan_sha256")?,
            sequence,
            state,
        })
    }
...
impl<'de> Deserialize<'de> for JournalRecord {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        if raw.schema_version != 1 {
            return Err(serde::de::Error::custom("unsupported journal schema"));
        }
        Ok(Self {
...
impl<'de> Deserialize<'de> for HistoryRecord {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        if raw.schema_version != 1 || raw.package_name.is_empty() {
            return Err(serde::de::Error::custom("invalid history record"));
        }
        Ok(Self {
            schema_version: raw.schema_version,
...
impl CanonicalDocument for HistoryRecord {
    fn from_canonical_json(bytes: &[u8]) -> Result<Self, DomainError> {
        ...
        if value.package_name.is_empty() {
            return Err(DomainError::Empty {
                field: "package_name",
            });
        }

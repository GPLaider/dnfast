...
use crate::{CanonicalDocument, DomainError, Evra, Sha256Digest, canonical};

#[derive(Clone, Debug, Eq, PartialEq, thiserror::Error)]
pub enum EraseLookupError {
    #[error("installed instance was not found")]
    NotFound,
    #[error("installed instance header digest changed")]
    HeaderMismatch,
    #[error("installed instance identity is ambiguous")]
    Ambiguous,
}
...
impl<'de> Deserialize<'de> for InstalledPackage {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        if raw.name.is_empty() {
            return Err(serde::de::Error::custom("empty package name"));
        }
        Ok(Self {
            name: raw.name,
            evra: raw.evra,
            vendor: raw.vendor,
            db_instance: raw.db_instance,
            install_time: raw.install_time,
            immutable_header_sha256: raw.immutable_header_sha256,
        })
    }
}
...
impl InstalledPackage {
    pub fn new(
        ...
        Ok(Self {
            ...
            db_instance,
            install_time,
            immutable_header_sha256: Sha256Digest::parse(digest, "immutable_header_sha256")?,
        })
    }
    pub fn name(&self) -> &str {
        &self.name
    }
    pub fn evra(&self) -> &Evra {
        &self.evra
    }
    pub fn vendor(&self) -> &str {
        &self.vendor
    }
    pub const fn db_instance(&self) -> u64 {
        self.db_instance
    }
    pub const fn install_time(&self) -> u64 {
        self.install_time
    }
    pub fn immutable_header_sha256(&self) -> &Sha256Digest {
        &self.immutable_header_sha256
    }
}
...
impl<'de> Deserialize<'de> for InstalledInventory {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        let value = Self {
            ...
            rpmdb_backend: raw.rpmdb_backend,
            rpm_version: raw.rpm_version,
...
impl InstalledInventory {
    pub fn new(
        ...
        let value = Self {
            schema_version: 1,
            install_root: "/".into(),
            rpmdb_backend: backend.into(),
            rpm_version: rpm_version.into(),
            packages,
            canonical_sha256: OnceLock::new(),
        };
        value.validate()?;
    ...
    pub fn erase_target(
        ...
        db_instance: u64,
        header_sha256: &str,
    ) -> Result<&InstalledPackage, EraseLookupError> {
        let matches = self
            .packages
            .iter()
            .filter(|package| package.db_instance == db_instance)
            .collect::<Vec<_>>();
    ...
    fn validate(&self) -> Result<(), DomainError> {
        ...
        if self.install_root != "/" {
            return Err(DomainError::InvalidPlan("inventory root must be /"));
        }
        if self.rpmdb_backend.is_empty() {
            return Err(DomainError::Empty {
                field: "rpmdb_backend",
            });
        }
        if self.rpm_version.is_empty() {
            return Err(DomainError::Empty {
                field: "rpm_version",
            });
        }
...
impl CanonicalDocument for InstalledInventory {
    ...
    fn canonical_sha256(&self) -> Result<Sha256Digest, DomainError> {
        if let Some(digest) = self.canonical_sha256.get() {

...
use crate::{CanonicalDocument, DomainError, Sha256Digest, canonical};

const SCHEMA_VERSION: u32 = 1;

#[derive(Clone, Copy, Debug, Deserialize, Eq, PartialEq, Serialize)]
#[serde(rename_all = "snake_case")]
pub enum SigningSubkeyRule {
    PrimaryOnly,
    AuthorizedSubkeys,
}
...
impl<'de> Deserialize<'de> for RepoTrustPolicy {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        let value = Self {
            ...
            repo_id: raw.repo_id,
            key_bundle_sha256: raw.key_bundle_sha256,
            allowed_primary_fingerprints: raw.allowed_primary_fingerprints,
            signing_subkey_rule: raw.signing_subkey_rule,
            valid_at_unix: raw.valid_at_unix,
...
impl RepoTrustPolicy {
    ...
    pub fn key_bundle_sha256(&self) -> &Sha256Digest {
        &self.key_bundle_sha256
    }
    pub fn allowed_primary_fingerprints(&self) -> &[String] {
        &self.allowed_primary_fingerprints
    }
    pub const fn signing_subkey_rule(&self) -> SigningSubkeyRule {
        self.signing_subkey_rule
    }
    pub const fn valid_at_unix(&self) -> u64 {
        self.valid_at_unix
    }
    pub fn new(
        ...
        let mut allowed_primary_fingerprints = fingerprints
            .into_iter()
    ...
    fn validate(&self) -> Result<(), DomainError> {
        ...
        if self.allowed_primary_fingerprints.is_empty() {
            return Err(DomainError::Empty {
                field: "allowed_primary_fingerprints",
            });
        }
        if self
            .allowed_primary_fingerprints
        ...
        if !self.allowed_primary_fingerprints.iter().all(|fingerprint| {
        ...
        if !self.require_package_signature {
            return Err(DomainError::UnsafeAction(
                "package signatures are mandatory",
            ));
        }
        Ok(())
    }

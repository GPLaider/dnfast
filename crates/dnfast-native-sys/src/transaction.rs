...
use crate::{empty_error, status_error, Context, Keyring, NativeError, RawContext, RawError, VerifiedPackage};

#[repr(C)]
#[derive(Clone, Copy)]
struct RawCounts {
    fd_open: u64, fd_close: u64, open_attempted: u64, open_failed: u64,
    rewind_attempted: u64, rewind_succeeded: u64, rewind_failed: u64,
    close_attempted: u64, close_failed: u64, script_start: u64, script_stop: u64,
    package_stop: u64, test_run: u64, real_run: u64,
}
...
pub struct TransactionCounts {
    pub fd_open: u64, pub fd_close: u64, pub script_start: u64,
    pub open_attempted: u64, pub open_failed: u64,
    pub rewind_attempted: u64, pub rewind_succeeded: u64, pub rewind_failed: u64,
    pub close_attempted: u64, pub close_failed: u64,
    pub script_stop: u64, pub package_stop: u64, pub test_run: u64,
    pub real_run: u64,
}

#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum TransactionPhase { Preflight, Started }

unsafe extern "C" {
    fn dnfast_transaction_add_install(context: *mut RawContext, keyring: *mut RawKeyring,
        fd: i32, expected: *const RawInstall, error: *mut RawError) -> i32;
    fn dnfast_transaction_add_erase(context: *mut RawContext, instance: u64,
        digest: *const u8, error: *mut RawError) -> i32;
    fn dnfast_transaction_prepare(context: *mut RawContext, error: *mut RawError) -> i32;
    fn dnfast_transaction_test(context: *mut RawContext, result: *mut i32, error: *mut RawError) -> i32;
    fn dnfast_transaction_run(context: *mut RawContext, result: *mut i32, error: *mut RawError) -> i32;
...
impl Context {
    pub fn transaction_add_install(&mut self, keyring: &Keyring, fd: i32,
        expected: &VerifiedPackage, digest: &[u8; 32], size: u64,
        upgrade: bool) -> Result<(), NativeError> {
        let expected = RawInstall { package: RawVerifiedPackage::from_verified(expected)?,
            artifact_sha256: *digest, artifact_size: size, upgrade: u8::from(upgrade) };
        ...
        let status = unsafe { dnfast_transaction_add_install(self.raw.as_ptr(),
            keyring.raw.as_ptr(), fd, &expected, &mut error) };
        if status == 0 { Ok(()) } else { Err(status_error(status, &mut error)) }
    }

    pub fn transaction_add_erase(&mut self, instance: u64,
        digest: &[u8; 32]) -> Result<(), NativeError> {
        let mut error = empty_error();
        // SAFETY: [Category 8 — FFI boundary UB] digest is an initialized fixed
        // array and the native context is uniquely borrowed.
        let status = unsafe { dnfast_transaction_add_erase(self.raw.as_ptr(),
            instance, digest.as_ptr(), &mut error) };
        if status == 0 { Ok(()) } else { Err(status_error(status, &mut error)) }
    }

    pub fn transaction_prepare(&mut self) -> Result<(), NativeError> {
        self.transaction_status(|context, error| unsafe { dnfast_transaction_prepare(context, error) })
    }
    ...
    pub fn transaction_verify_db(&mut self) -> Result<(), NativeError> {
        self.transaction_status(|context, error| unsafe { dnfast_transaction_verify_db(context, error) })
    }
    ...
    pub fn transaction_problems(&self) -> Result<Vec<String>, NativeError> {
        ...
        (0..count).map(|index| {
            ...
            if value.is_null() { return Err(invalid_result()); }
            // SAFETY: successful native problem entries are NUL terminated.
            unsafe { CStr::from_ptr(value) }.to_str().map(str::to_owned).map_err(|_| invalid_result())
        }).collect()
    }
    ...
    pub fn transaction_counts(&self) -> TransactionCounts {
        ...
        TransactionCounts { fd_open: value.fd_open, fd_close: value.fd_close,
    ...
    fn transaction_status(&mut self,
        call: impl FnOnce(*mut RawContext, *mut RawError) -> i32) -> Result<(), NativeError> {
        let mut error = empty_error();
        let status = call(self.raw.as_ptr(), &mut error);
        if status == 0 { Ok(()) } else { Err(status_error(status, &mut error)) }
    }

    fn transaction_result(&mut self,
        call: impl FnOnce(*mut RawContext, *mut i32, *mut RawError) -> i32) -> Result<i32, NativeError> {
        let mut error = empty_error();
        let mut result = 0;
...
fn invalid_result() -> NativeError {
    NativeError { status: 7, component: "rpm".into(), symbol: "transaction".into(),

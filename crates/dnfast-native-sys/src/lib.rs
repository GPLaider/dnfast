...
mod authority;
pub use authority::{fork_probe as authority_fork_probe, Authority};
mod executor_fd;
...
unsafe extern "C" {
    ...
    fn dnfast_solver_add_repo(context: *mut RawContext, input: *const RawRepoInput, error: *mut RawError) -> i32;
    fn dnfast_solver_add_rpmdb(context: *mut RawContext, root: *const c_char, error: *mut RawError) -> i32;
    fn dnfast_solver_solve_operation(context: *mut RawContext, request: *const RawSolveRequest, operation: u8, error: *mut RawError) -> i32;
    ...
    fn dnfast_solver_action_requested_spec(context: *const RawContext, index: usize) -> *const c_char;
    fn dnfast_solver_action_requested_relation_kind(context: *const RawContext, index: usize) -> u8;
...
#[derive(Clone, Copy, Debug, Eq, PartialEq)]
pub enum SolveOperation { Install, Erase, Upgrade }
...
impl Context {
    pub fn open(architecture: PoolArchitecture, interrupt: impl FnMut() -> bool + 'static) -> Result<Self, NativeError> {
        Self::open_with_limits(architecture, interrupt, Limits::default())
    }

    pub fn open_with_limits(architecture: PoolArchitecture, interrupt: impl FnMut() -> bool + 'static, configured: Limits) -> Result<Self, NativeError> {
        let mut callback = Box::new(CallbackState { interrupt: Box::new(interrupt),
        ...
        let limits = RawLimits { abi_version: ABI_VERSION, max_packages: configured.max_packages, max_relations_per_package: configured.max_relations_per_package, pool_architecture: architecture as u32, max_metadata_bytes: configured.max_metadata_bytes };
        let _ = defaults;
        ...
        let mut error = RawError { status: 0, component: std::ptr::null_mut(), symbol: std::ptr::null_mut(), message: std::ptr::null_mut() };
        ...
        let raw = NonNull::new(raw).ok_or_else(|| NativeError {
            status: 7, component: "dnfast".into(), symbol: String::new(), message: "native success returned null context".into(),
        })?;
        Ok(Self { raw, _callback: callback, _thread_affine: PhantomData })
    }
    ...
    pub fn pool_architecture(&self) -> Result<PoolArchitecture, NativeError> {
        ...
        if value.is_null() {
            return Err(NativeError { status: 7, component: "dnfast".into(), symbol: "pool_architecture".into(), message: "native pool architecture was null".into() });
        }
        match unsafe { CStr::from_ptr(value) }.to_bytes() {
            b"aarch64" => Ok(PoolArchitecture::Aarch64),
            b"x86_64" => Ok(PoolArchitecture::X86_64),
            _ => Err(NativeError { status: 7, component: "dnfast".into(), symbol: "pool_architecture".into(), message: "native pool architecture was unsupported".into() }),
        }
    }
    ...
    fn add_repo_kind(&mut self, repo: &RepoInput, installed: bool) -> Result<(), NativeError> {
        ...
        let input = RawRepoInput { abi_version: ABI_VERSION, id: id.as_ptr(), repomd_path: repomd.as_ptr(), primary_path: primary.as_ptr(), filelists_path: filelists.as_ptr(), priority: repo.priority, cost: repo.cost, installed: u8::from(installed) };
    ...
    pub fn add_rpmdb(&mut self, root: &str) -> Result<(), NativeError> {
        ...
        let status = unsafe { dnfast_solver_add_rpmdb(self.raw.as_ptr(), root.as_ptr(), &mut error) };
        if status == 0 { Ok(()) } else { Err(status_error(status, &mut error)) }
    }

    pub fn solve_install(&mut self, name: &str, weak: bool, best: bool) -> Result<SolveOutput, NativeError> {
        self.solve_install_many(&[name], weak, best)
    }

    pub fn solve_install_many(&mut self, names: &[&str], weak: bool, best: bool) -> Result<SolveOutput, NativeError> {
        self.solve_with_operation(names, weak, best, SolveOperation::Install)
    }

    pub fn solve_with_operation(&mut self, names: &[&str], weak: bool, best: bool, operation: SolveOperation) -> Result<SolveOutput, NativeError> {
        if names.is_empty() && operation != SolveOperation::Upgrade {
            return Err(NativeError { status: 1, component: "dnfast".into(), symbol: String::new(), message: "empty solve request".into() });
        }
        let strings: Vec<_> = names.iter().map(|name| c_string(name)).collect::<Result<_, _>>()?;
        let pointers: Vec<_> = strings.iter().map(|name| name.as_ptr()).collect();
        let request = RawSolveRequest { abi_version: ABI_VERSION, names: pointers.as_ptr(), name_count: pointers.len(), install_weak_deps: u8::from(weak), best: u8::from(best) };
        let mut error = empty_error();
        let operation = match operation { SolveOperation::Install => 0, SolveOperation::Erase => 1, SolveOperation::Upgrade => 2 };
        ...
        let status = unsafe { dnfast_solver_solve_operation(self.raw.as_ptr(), &request, operation, &mut error) };
        if status != 0 { return Err(status_error(status, &mut error)); }
        Ok(SolveOutput { actions: copy_items(self.raw, 0)?, repositories: copy_items(self.raw, 1)?, kinds: copy_items(self.raw, 2)?,
            obsoletes: copy_obsoletes(self.raw), requested_specs: copy_requested_specs(self.raw)?,
            requested_relation_kinds: copy_requested_relation_kinds(self.raw)?,
            problems: copy_items(self.raw, 3)?, decisions: copy_decisions(self.raw)? })
    }
}

#[derive(Debug, Clone, Copy, Eq, PartialEq)]
pub struct Limits { pub max_packages: u32, pub max_relations_per_package: u32, pub max_metadata_bytes: u64 }

impl Default for Limits {
    fn default() -> Self { Self { max_packages: 2_000_000, max_relations_per_package: 4_096, max_metadata_bytes: 17_179_869_184 } }
}

#[derive(Debug, Clone, Eq, PartialEq)]
pub struct RepoInput { pub id: String, pub repomd_path: String, pub primary_path: String, pub filelists_path: String, pub priority: i32, pub cost: i32 }

#[derive(Debug, Clone, Eq, PartialEq)]
pub struct SolveOutput { pub actions: Vec<String>, pub repositories: Vec<String>, pub kinds: Vec<String>, pub obsoletes: Vec<Option<String>>, pub requested_specs: Vec<Option<String>>, pub requested_relation_kinds: Vec<bool>, pub problems: Vec<String>, pub decisions: Vec<DecisionOutput> }

#[derive(Debug, Clone, Eq, PartialEq)]
pub struct DecisionOutput { pub requiring: String, pub provider: String, pub relation: String, pub weak: bool, pub provider_installed: bool }
...
mod tests {
    ...
    fn x86_64_context_selects_the_x86_64_native_pool() {
        ...
        assert_eq!(context.pool_architecture().unwrap(), PoolArchitecture::X86_64);
    }
...
pub(crate) fn empty_error() -> RawError { RawError { status: 0, component: std::ptr::null_mut(), symbol: std::ptr::null_mut(), message: std::ptr::null_mut() } }

pub(crate) fn status_error(status: i32, raw: &mut RawError) -> NativeError {
    if raw.status != 0 { take_error(raw) } else { NativeError { status, component: "dnfast".into(), symbol: String::new(), message: "native operation interrupted".into() } }
}

pub(crate) fn c_string(value: &str) -> Result<CString, NativeError> {
    CString::new(value).map_err(|_| NativeError { status: 1, component: "dnfast".into(), symbol: String::new(), message: "string contains NUL".into() })
}
...
fn copy_items(raw: NonNull<RawContext>, mode: u8) -> Result<Vec<String>, NativeError> {
    ...
    let count = unsafe { if mode < 3 { dnfast_solver_action_count(raw.as_ptr()) } else { dnfast_solver_problem_count(raw.as_ptr()) } };
    let mut output = Vec::with_capacity(count);
    for index in 0..count {
        // SAFETY: same invariant as above; index is bounded by native count.
        let item = unsafe { match mode { 0 => dnfast_solver_action(raw.as_ptr(), index), 1 => dnfast_solver_action_repo(raw.as_ptr(), index), 2 => dnfast_solver_action_kind(raw.as_ptr(), index), _ => dnfast_solver_problem(raw.as_ptr(), index) } };
        if item.is_null() { return Err(NativeError { status: 7, component: "dnfast".into(), symbol: "result".into(), message: "native result was null".into() }); }
        // SAFETY: native contract guarantees a valid NUL string for every item.
        output.push(unsafe { CStr::from_ptr(item) }.to_string_lossy().into_owned());
    }
    Ok(output)
}
...
fn copy_requested_relation_kinds(raw: NonNull<RawContext>) -> Result<Vec<bool>, NativeError> {
    ...
    (0..count).map(|index| {
        ...
        match unsafe { dnfast_solver_action_requested_relation_kind(raw.as_ptr(), index) } {
            0 => Ok(false),
            1 => Ok(true),
            _ => Err(NativeError { status: 7, component: "dnfast".into(), symbol: "requested_relation_kind".into(),
...
fn copy_requested_spec(pointer: *const c_char) -> Result<Option<String>, NativeError> {
    ...
    unsafe { CStr::from_ptr(pointer) }.to_str().map(|value| Some(value.to_owned())).map_err(|_| NativeError {
        status: 7, component: "dnfast".into(), symbol: "requested_spec".into(),
...
fn copy_decisions(raw: NonNull<RawContext>) -> Result<Vec<DecisionOutput>, NativeError> {
    ...
    for index in 0..count {
        ...
        let pointers = unsafe { [dnfast_solver_decision_requiring(raw.as_ptr(), index), dnfast_solver_decision_provider(raw.as_ptr(), index), dnfast_solver_decision_relation(raw.as_ptr(), index)] };
        if pointers.iter().any(|item| item.is_null()) { return Err(NativeError { status: 7, component: "dnfast".into(), symbol: "decision".into(), message: "native decision was null".into() }); }
        let text = |pointer| {
        ...
        let (kind, installed) = unsafe { (dnfast_solver_decision_kind(raw.as_ptr(), index), dnfast_solver_decision_provider_installed(raw.as_ptr(), index)) };
        output.push(DecisionOutput { requiring: text(pointers[0]), provider: text(pointers[1]), relation: text(pointers[2]), weak: kind == 1, provider_installed: installed == 1 });
    }
    Ok(output)
}
...
fn take_error(raw: &mut RawError) -> NativeError {
    ...
    let error = NativeError { status: raw.status, component: text(raw.component), symbol: text(raw.symbol), message: text(raw.message) };

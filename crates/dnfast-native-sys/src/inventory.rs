...
use crate::{
    Context, Keyring, NativeError, RawContext, RawError, c_string, empty_error, status_error,
};
...
unsafe extern "C" {
    fn dnfast_inventory_read_cached(
    ...
    fn dnfast_inventory_write_begin(
        context: *mut RawContext,
    ...
    fn dnfast_inventory_read_locked_cached(
        context: *mut RawContext,
        expected_cookie: *const c_char,
        cache_hit: *mut u8,
        error: *mut RawError,
    ...
    fn dnfast_inventory_test_run(
        context: *mut RawContext,
        result: *mut i32,
        error: *mut RawError,
    ) -> i32;
    fn dnfast_inventory_run(
        context: *mut RawContext,
        result: *mut i32,
        error: *mut RawError,
    ) -> i32;
...
impl Context {
    pub fn read_inventory(&mut self, root: &str) -> Result<Inventory, NativeError> {
        self.read_inventory_cached(root, None)?
            .inventory
            .ok_or_else(null_result)
    }

    pub fn read_inventory_cached(
        &mut self,
        root: &str,
        ...
        let inventory = if cache_hit == 0 {
            Some(self.copy_inventory()?)
        } else {
            None
        };
    ...
    pub fn read_locked_inventory(&mut self) -> Result<Inventory, NativeError> {
        self.read_locked_inventory_cached(None)?
            .inventory
            .ok_or_else(null_result)
    }

    pub fn read_locked_inventory_cached(
        &mut self,
        expected_cookie: Option<&str>,
    ) -> Result<InventoryRead, NativeError> {
        let expected_cookie = expected_cookie.map(c_string).transpose()?;
        ...
        let status = unsafe {
            dnfast_inventory_read_locked_cached(
                self.raw.as_ptr(),
                expected_cookie
                    .as_ref()
                    .map_or(std::ptr::null(), |value| value.as_ptr()),
    ...
    pub fn run_counts(&self) -> (u64, u64) {
        ...
        unsafe {
            (
                dnfast_inventory_test_count(self.raw.as_ptr()),
                dnfast_inventory_real_count(self.raw.as_ptr()),
            )
        }
    }
    ...
    pub fn test_run(&mut self) -> Result<i32, NativeError> {
        ...
        let status =
            unsafe { dnfast_inventory_test_run(self.raw.as_ptr(), &mut result, &mut error) };
        if status == 0 {
            Ok(result)
        } else {
            Err(status_error(status, &mut error))
    ...
    pub fn inventory_call_order(&self) -> (u64, u64) {
        ...
        unsafe {
            (
                dnfast_inventory_keyring_sequence(self.raw.as_ptr()),
                dnfast_inventory_rpmdb_sequence(self.raw.as_ptr()),
            )
        }
    }
    ...
    fn copy_inventory(&self) -> Result<Inventory, NativeError> {
        ...
        for index in 0..count {
            packages.push(self.copy_package(index)?);
        }
        Ok(Inventory {
            backend,
            rpm_version,
    ...
    fn copy_package(&self, index: usize) -> Result<InventoryPackage, NativeError> {
        ...
        if item.immutable_header.is_null() || item.immutable_header_size == 0 {
            return Err(null_result());
        }
        // SAFETY: native contract exposes this exact initialized byte range.
        let bytes = unsafe {
            std::slice::from_raw_parts(item.immutable_header, item.immutable_header_size)
        };
        Ok(InventoryPackage {
...
fn null_result() -> NativeError {
    NativeError {
        status: 7,
        component: "dnfast".into(),
        symbol: "inventory".into(),
        message: "native inventory result was null".into(),
    }
}
...
pub(crate) unsafe fn fixture_global_counts() -> (u64, u64) {
    ...
    unsafe {
        (
            dnfast_inventory_fixture_global_test_count(),
            dnfast_inventory_fixture_global_real_count(),
        )
    }
}

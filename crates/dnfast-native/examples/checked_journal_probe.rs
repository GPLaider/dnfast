use std::{fs, os::unix::fs::PermissionsExt, path::{Path, PathBuf}, rc::Rc,
    sync::Arc, time::{SystemTime, UNIX_EPOCH}};

use dnfast_cache::{ArtifactCache, ArtifactError, ArtifactResponse, ArtifactSpec,
    ArtifactTransport, Digest, TransactionRequest};
use dnfast_core::{RepoTrustPolicy, SigningSubkeyRule};
use dnfast_native::{ExecutorInventory, ExpectedPackage, InventoryError,
    InventoryReader, KeyringInstalled};
...
impl ArtifactTransport for FileTransport {
    fn open(&self, _: &str) -> Result<ArtifactResponse, ArtifactError> {
        Ok(ArtifactResponse { status: 200, body: Box::new(fs::File::open(&self.0)
...
fn run() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let root = PathBuf::from(format!("/dev/shm/dnfast-journal-probe-{}", std::process::id()));
    let repo = format!("journal-{}", std::process::id());
    private(Path::new("/etc/dnfast"))?; private(Path::new("/etc/dnfast/keys"))?;
    let keys = PathBuf::from("/etc/dnfast/keys").join(&repo); private(&keys)?;
    let key = keys.join("allowed.asc"); fs::copy(key_source, &key)?;
    fs::set_permissions(&key, fs::Permissions::from_mode(0o600))?;
    let bundle = dnfast_repo::key_bundle_digest(&repo, std::slice::from_ref(&key))?;
    let policy = RepoTrustPolicy::new(&repo, hex::encode(bundle.digest),
    ...
    let spec = ArtifactSpec::new("https://fixture.invalid/", "https://fixture.invalid/",
        "package.rpm", Digest::Sha256(digest.clone()), size)?;
    ...
    let verified = keyring.verify_artifact(&artifact, &ExpectedPackage { name: "dnfast-noarch".into(),
        epoch: 0, version: "1.0".into(), release: "1".into(), arch: "noarch".into(), vendor: "unknown".into() },
        SigningSubkeyRule::AuthorizedSubkeys)?;
    ...
    let mut rejected = ExecutorInventory::begin(dnfast_core::Architecture::Aarch64, KeyringInstalled::from_repository(
        &policy, &repo, std::slice::from_ref(&key))?, &before)?;
    rejected.add_install(&artifact, &verified, false)?;
    rejected.bind_journal(Rc::clone(&failed))?;
    rejected.prepare_checked_transaction()?; rejected.test_checked_transaction()?;
    assert!(matches!(rejected.run_checked_transaction(),
        Err(InventoryError::TransactionPreflight { .. } | InventoryError::Native(_))));
    assert_eq!(rejected.transaction_counts().real_run, 0);
    assert_eq!(failed.entries()?.last().unwrap().state, TransactionState::Prepared);
    drop(rejected);
    ...
    let mut execution = ExecutorInventory::begin(dnfast_core::Architecture::Aarch64, keyring, &before)?;
    ...
    println!("journal_abort_real0=true durable_started_before_real=true result_after_real=true authority_all_phases={}",
        execution.fixture_authority_is_held());
    drop(execution);
    let after = reader.read()?;
    let mut replacement = ExecutorInventory::begin(dnfast_core::Architecture::Aarch64, KeyringInstalled::from_repository(
        &policy, &repo, std::slice::from_ref(&key))?, &after)?;

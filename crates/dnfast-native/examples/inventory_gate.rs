...
fn main() {
    ...
    if arguments.first().map(String::as_str) == Some("--test-fail") {
        let mut execution = ExecutorInventory::begin(
        ...
        assert!(matches!(
            execution.test_transaction(),
            Err(InventoryError::TestFailed(-99))
        ));
        ...
        let mut holder = std::process::Command::new(&arguments[1])
            .arg(&arguments[2])
            .spawn()
            .expect("lock holder");
        let deadline = std::time::Instant::now() + std::time::Duration::from_secs(2);
        while !std::path::Path::new(&arguments[2]).exists() && std::time::Instant::now() < deadline
        {
        ...
        assert!(matches!(
            ExecutorInventory::begin_interruptible(
                dnfast_core::Architecture::Aarch64,
                KeyringInstalled::fixture().expect("fixture keyring"),
                &expected,
                interrupted
        ...
        let mut replacement = ExecutorInventory::begin(
            dnfast_core::Architecture::Aarch64,
            KeyringInstalled::fixture().expect("replacement keyring"),
            &expected,
        )
        .expect("released authority and context");
        ...
        println!(
            "keyring_sequence={} rpmdb_sequence={} test_result={test_result} test_count={test_count} run_result={run_result} too_late_held=true",
            order.0, order.1
        );
    }

...
use crate::{
    Refresher, Source,
    tests::{FakeTransport, metadata_fixture, metalink_fixture},
};
...
fn every_source_kind_publishes_identical_repomd_digest() {
    ...
    let direct = FakeTransport::new([
        ...
        (
            format!("{base}/repodata/filelists.xml.zst"),
            filelists.clone(),
        ),
    ]);
    let direct = Refresher::new(direct, &direct_cache)
        .refresh("repo", Source::BaseUrl(base.into()))
        .unwrap()
        .digest;
    ...
    let metalink = Refresher::new(metalink, &metalink_cache)
        .refresh("repo", Source::Metalink("https://meta.example/list".into()))
        .unwrap()
        .digest;
    ...
    let mirror = FakeTransport::new([
        (
            ...
            b"https://mirror.example/repo\n".to_vec(),
        ),
        (
            "https://mirror.example/repo/repodata/repomd.xml".into(),
            repomd,
        ),
        (
            "https://mirror.example/repo/repodata/primary.xml.zst".into(),
            primary,
        ),
        (
            "https://mirror.example/repo/repodata/filelists.xml.zst".into(),
            filelists,
        ),
    ]);
    let mirror = Refresher::new(mirror, &mirror_cache)
        .refresh(
            "repo",

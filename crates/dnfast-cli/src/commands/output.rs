...
pub(super) fn write_new_plan(path: &Path, bytes: &[u8]) -> Result<(), AppFailure> {
    ...
    let relative = path
        .strip_prefix("/")
        .map_err(|_| invalid_path("output path must be absolute"))?;
    let root = open(
        "/",
...
pub(super) fn validate_new_path(path: &Path) -> Result<(), AppFailure> {
    let raw = path
        .to_str()
        .ok_or_else(|| invalid_path("output path is not UTF-8"))?;
    if !path.is_absolute() || raw.chars().any(char::is_control) {
        return Err(invalid_path(
            "output path must be absolute UTF-8 without control characters",
        ));
    }
    let relative = path
        .strip_prefix("/")
...
fn invalid_path(message: &str) -> AppFailure {
    AppFailure::with_error_code(2, "invalid_output_path", message)
}
fn errno_failure(error: rustix::io::Errno) -> AppFailure {
    AppFailure::new(1, error.to_string())
}
fn std_io_failure(error: std::io::Error) -> AppFailure {
    AppFailure::new(1, error.to_string())
}

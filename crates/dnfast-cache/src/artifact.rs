...
impl ArtifactSpec {
    pub fn new(
        ...
        if origin(&repository) != origin(&selected) {
            return Err(ArtifactError::Policy("selected URL is not same-origin".into()));
        }
    ...
    fn from_selected_base(
        ...
        if location.is_empty()
            ...
            || location.contains('%')
            || location.contains("://")
            || location.split('/').any(|part| part.is_empty() || matches!(part, "." | ".."))
        {
            return Err(ArtifactError::Policy("unsafe metadata package location".into()));
        }
        let Digest::Sha256(value) = digest;
        if value.len() != 64 || !value.bytes().all(|byte| byte.is_ascii_hexdigit()) {
            return Err(ArtifactError::Policy("unsupported SHA-256 digest".into()));
        }
        let url = selected.join(location).map_err(|error| ArtifactError::Policy(error.to_string()))?;
        if origin(&url) != origin(&selected) {
            return Err(ArtifactError::Policy("package URL escaped selected origin".into()));
        }
        Ok(Self { url: url.into(), digest: value.to_ascii_lowercase(), size })
    }
}
...
fn parse_base(value: &str) -> Result<Url, ArtifactError> {
    ...
    if url.scheme() != "https" || url.username() != "" || url.password().is_some() || url.host_str().is_none() || url.query().is_some() || url.fragment().is_some() {
...
fn origin(url: &Url) -> (&str, Option<u16>, &str) {
    (url.scheme(), url.port_or_known_default(), url.host_str().unwrap_or_default())
}
...
impl TransactionRequest {
    pub fn from_totals(predicted_bytes: u64, artifacts: u64) -> Result<Self, ArtifactError> {
        let request = Self { predicted_bytes, artifacts, identities: Vec::new() };
        request.validate(Capacity { cached_bytes: 0, available_bytes: u64::MAX })?;
        Ok(request)
    }

    pub fn for_specs(specs: &[ArtifactSpec]) -> Result<Self, ArtifactError> {
        let predicted_bytes = specs.iter().try_fold(0_u64, |total, spec| total.checked_add(spec.size)).ok_or_else(|| ArtifactError::Capacity("transaction byte count overflow".into()))?;
        let artifacts = u64::try_from(specs.len()).map_err(|error| ArtifactError::Capacity(error.to_string()))?;
        let mut identities = specs.iter().map(|spec| (spec.digest.clone(), spec.size)).collect::<Vec<_>>();
        identities.sort();
        if identities.windows(2).any(|window| window[0] == window[1]) {
            return Err(ArtifactError::Capacity("duplicate artifact in transaction".into()));
        }
    ...
    pub fn validate(&self, capacity: Capacity) -> Result<(), ArtifactError> {
        if self.predicted_bytes > MAX_TRANSACTION_BYTES || self.artifacts > MAX_TRANSACTION_ARTIFACTS {
            return Err(ArtifactError::Capacity("transaction artifact limit exceeded".into()));
        }
        let cache_after = capacity.cached_bytes.checked_add(self.predicted_bytes).ok_or_else(|| ArtifactError::Capacity("cache size overflow".into()))?;
        if cache_after > MAX_CACHE_BYTES {
            return Err(ArtifactError::Capacity("artifact cache cap exceeded".into()));
        }
        let reserve = self.predicted_bytes.checked_add(self.predicted_bytes.checked_add(19).ok_or_else(|| ArtifactError::Capacity("reserve overflow".into()))? / 20).ok_or_else(|| ArtifactError::Capacity("reserve overflow".into()))?;
        if capacity.available_bytes < reserve {
            return Err(ArtifactError::Capacity("filesystem reserve is insufficient".into()));
        }
        Ok(())
    }
}
...
pub struct HttpArtifactTransport { agent: ureq::Agent }
...
impl HttpArtifactTransport {
    ...
    pub fn with_root_certificate_pem(pem: &[u8]) -> Result<Self, ArtifactError> {
        ...
        Ok(Self::with_tls(ureq::tls::RootCerts::new_with_certs(&[certificate])))
    }
...
impl ArtifactTransport for HttpArtifactTransport {
    fn open(&self, url: &str) -> Result<ArtifactResponse, ArtifactError> {
        let response = self.agent.get(url).header("Accept-Encoding", "identity").call().map_err(|error| ArtifactError::Transport(error.to_string()))?;
        let status = response.status().as_u16();
        Ok(ArtifactResponse { status, body: Box::new(response.into_body().into_reader()) })
    }
}

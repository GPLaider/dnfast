...
use dnfast_metadata::{
    decode_primary, decode_record, parse_filelists_record, parse_primary, parse_primary_records,
    parse_repomd, parse_repomd_records, validate_filelists_generation, CompletePackage,
    FileListPackage, Package,
};
...
impl Cache {
    pub fn publish(&self, repository: &str, repomd: &[u8], primary: &[u8]) -> Result<Snapshot, CacheError> {
    ...
    pub fn publish_complete_with_origin_and_authentication(
        ...
        let solver_inputs = parse_primary_records(primary_open.as_slice()).map_err(metadata_error)?;
        let filelist_inputs = parse_filelists_record(filelists, &records.filelists).map_err(metadata_error)?;
    ...
    fn write_object(&self, objects: &std::path::Path, object: &std::path::Path, input: &Publication<'_>) -> Result<(), CacheError> {
        let staging = tempfile::Builder::new().prefix(".staging-").tempdir_in(objects).map_err(io_error)?;
        ...
        let mut manifest = Manifest {
            ...
            repository, integrity: input.integrity, filelists: None, filelists_index: None,
            solver_inputs: None,
            source_origin: None,
        };
        if let (Some(bytes), Some(solver), Some(files)) = (input.filelists_bytes, input.solver_inputs, input.filelists) {
            manifest.filelists = Some(write_verified(staging.path(), "filelists", bytes)?);
            manifest.solver_inputs = Some(write_verified(staging.path(), "solver-inputs.json", &json(solver)?)?);
            manifest.filelists_index = Some(write_verified(staging.path(), "filelists-index.json", &json(files)?)?);
        }
        if let Some(origin) = input.source_origin { manifest.source_origin = Some(write_verified(staging.path(), "source-origin", origin.as_bytes())?); }
        ...
        match fs::rename(staging.path(), object) {
            Ok(()) => { std::mem::forget(staging); sync_directory(objects) }
            Err(_error) if object.exists() => self.open_by_digest(&sha256(input.repomd)).map(|_| ()),
    ...
    fn publish_repository_pointer(&self, repository: &str, digest: &str,
        ...
        if id_path.exists() {
            let stored = String::from_utf8(crate::fs_safety::read_regular(&id_path)?)
                .map_err(|error| CacheError::Corrupt(error.to_string()))?;
            if stored != repository { return Err(CacheError::Corrupt("repository identity mismatch".into())); }
        } else {
...
fn search_package(record: &CompletePackage) -> Package {
    Package { name: record.name.clone(), arch: record.arch.clone(), epoch: record.epoch.clone(), version: record.version.clone(), release: record.release.clone(), summary: record.summary.clone() }
}
...
fn fault(root: &std::path::Path, name: &str) -> Result<(), CacheError> {
    if root.join(name).exists() { return Err(CacheError::Io(format!("injected fault: {name}"))); }
    Ok(())
}

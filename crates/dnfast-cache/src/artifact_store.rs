...
use rustix::fs::{
    AtFlags, Mode, OFlags, fstat, fstatvfs, fsync, linkat, open, openat, statat, unlinkat,
};
...
use crate::{
    ...
    artifact_lock::{
        AuthoritySet, ProcessReservation, acquire_authorities, lock_bounded, validate_cache_path,
...
impl CachedArtifact {
    ...
    pub fn from_verified_root_file(
        file: fs::File,
        sha256: &str,
        size: u64,
    ) -> Result<Self, ArtifactError> {
        Ok(Self {
            file: verify_identity(file.into(), sha256, size)?,
        })
    }
}
    ...
    pub fn begin_transaction(
        ...
        if transaction.identities.is_empty() {
            return Err(ArtifactError::Capacity(
                "transaction has no bound artifacts".into(),
            ));
        }
        ...
        let artifact_count = u64::try_from(transaction.identities.len())
...
impl ArtifactTransaction {
    pub fn fetch(
        ...
        if !self.remaining.contains(&identity) {
            return Err(ArtifactError::Capacity(
                "artifact is not pending in this transaction".into(),
            ));
        }
...
impl PinnedDirectory {
    ...
    fn capacity(&self) -> Result<Capacity, ArtifactError> {
        ...
        let available_bytes = filesystem
            ...
            .ok_or_else(|| ArtifactError::Capacity("available filesystem bytes overflow".into()))?;
        Ok(Capacity {
    ...
    fn verify_lock_identity(&self) -> Result<(), ArtifactError> {
        ...
        let resolved =
            statat(&self.fd, ".transaction-lock", AtFlags::SYMLINK_NOFOLLOW).map_err(errno)?;
        validate_lock_metadata(&resolved)?;
        if retained.st_dev != resolved.st_dev || retained.st_ino != resolved.st_ino {
            return Err(ArtifactError::Io(
                "artifact transaction lock changed".into(),
            ));
        }
        Ok(())
    }
    ...
    fn missing_bytes(&self, identities: &[(String, u64)]) -> Result<u64, ArtifactError> {
        identities.iter().try_fold(0_u64, |total, (digest, size)| {
            match openat(
                ...
                Ok(fd) => {
                    verify_identity(fd, digest, *size)?;
                    Ok(total)
                }
                Err(rustix::io::Errno::NOENT) => total.checked_add(*size).ok_or_else(|| {
                    ArtifactError::Capacity("missing artifact bytes overflow".into())
                }),
    ...
    fn cached_bytes(&self) -> Result<u64, ArtifactError> {
        fs::read_dir(format!("/proc/self/fd/{}", self.fd.as_fd().as_raw_fd()))
            .map_err(io_error)?
            .try_fold(0_u64, |total, entry| {
                ...
                total
                    ...
                    .ok_or_else(|| ArtifactError::Capacity("cached byte count overflow".into()))
            })
    }
    ...
    fn verify_path_identity(&self) -> Result<(), ArtifactError> {
        ...
        if metadata.st_dev != self.device || metadata.st_ino != self.inode {
            return Err(ArtifactError::Io(
                "artifact directory changed during transfer".into(),
            ));
        }
        Ok(())
    }
...
fn stream_verified(
    ...
    loop {
        ...
        total = total
            ...
            .ok_or_else(|| ArtifactError::Integrity("artifact size overflow".into()))?;
        if total > spec.size {
            return Err(ArtifactError::Integrity(
                "artifact exceeds declared size".into(),
            ));
        }
    ...
    if total != spec.size {
        return Err(ArtifactError::Integrity("artifact is truncated".into()));
    }
    if hex::encode(hasher.finalize()) != spec.digest {
        return Err(ArtifactError::Integrity("artifact digest mismatch".into()));
    }
    Ok(())
}
...
fn verify_identity(fd: OwnedFd, digest: &str, size: u64) -> Result<fs::File, ArtifactError> {
    ...
    {
        return Err(ArtifactError::Integrity(
            "existing artifact metadata mismatch".into(),
        ));
    }
    ...
    if hex::encode(hasher.finalize()) != digest {
        return Err(ArtifactError::Integrity(
            "existing artifact digest mismatch".into(),
        ));
    }
...
fn cache_error(error: crate::CacheError) -> ArtifactError {
    ArtifactError::Io(error.to_string())
}

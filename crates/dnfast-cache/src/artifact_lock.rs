...
use rustix::fs::{AtFlags, FlockOperation, Mode, OFlags, fcntl_lock, fstat, openat, unlinkat};
...
impl ProcessReservation {
    pub(crate) fn acquire(directory: &OwnedFd) -> Result<Self, ArtifactError> {
        ...
        match openat(
            ...
            Ok(_) => Ok(Self {
                directory: rustix::io::dup(directory).map_err(errno)?,
                name,
                pid,
            }),
            Err(rustix::io::Errno::EXIST) => Err(ArtifactError::Busy(
                "artifact transaction already active in this process".into(),
            )),
        ...
        match fcntl_lock(fd, FlockOperation::NonBlockingLockExclusive) {
            ...
            Err(rustix::io::Errno::WOULDBLOCK) => {
                return Err(ArtifactError::Busy(
                    "artifact transaction lock timed out".into(),
                ));
            }
...
pub(crate) fn acquire_authorities(
    root: &Path,
    directory: &OwnedFd,
) -> Result<AuthoritySet, ArtifactError> {
    ...
    let path_name = authority_name(
        b"dnfast-path-authority-v1",
        &[&uid, absolute.as_os_str().as_bytes()],
    );
    let identity_name = authority_name(
        b"dnfast-inode-authority-v1",
        &[
            &uid,
            &metadata.st_dev.to_be_bytes(),
            &metadata.st_ino.to_be_bytes(),
        ],
    );
...
fn acquire_until(
    name: &str,
    deadline: Instant,
) -> Result<dnfast_native_sys::Authority, ArtifactError> {
    loop {
        match dnfast_native_sys::Authority::acquire(name.as_bytes()) {
            ...
            Err(98) => {
                return Err(ArtifactError::Busy(
                    "artifact path authority timed out".into(),
                ));
            }
            Err(error) => {
                return Err(ArtifactError::Io(format!(
                    "artifact path authority error: {error}"
                )));
            }
        }
    }
...
fn normalize_cache_path(root: &Path) -> Result<PathBuf, ArtifactError> {
        ...
        match component {
            ...
            Component::ParentDir => {
                if !normalized.pop() {
                    return Err(ArtifactError::Policy(
                        "artifact cache path crosses root".into(),
                    ));
                }
            }
            Component::Normal(value) => {
                ...
                if bytes.contains(&0) || value.to_str().is_none() {
                    return Err(ArtifactError::Policy(
                        "artifact cache path is not safe UTF-8".into(),
                    ));
                }
            ...
            Component::Prefix(_) => {
                return Err(ArtifactError::Policy(
                    "unsupported artifact cache path prefix".into(),
                ));
            }
        }
    }
    if normalized == Path::new("/") {
        return Err(ArtifactError::Policy(
            "artifact cache root cannot be filesystem root".into(),
        ));
    }
    Ok(normalized)
}
...
fn process_start(pid: i32) -> Result<String, ArtifactError> {
    ...
    let fields = stat
        .rsplit_once(')')
        .ok_or_else(|| ArtifactError::Io("invalid process stat".into()))?
        .1;
    fields
        .split_whitespace()
        .nth(19)
        .map(str::to_owned)
        .ok_or_else(|| ArtifactError::Io("process start time missing".into()))
}

pub(crate) fn validate_owner_marker(
    directory: &OwnedFd,
    name: &std::ffi::OsStr,
) -> Result<(), ArtifactError> {
    let value = name
        .to_str()
        .ok_or_else(|| ArtifactError::Io("non-UTF-8 transaction owner marker".into()))?;
    ...
    if process_start(pid).is_ok_and(|actual| actual == start) {
        ...
        Err(ArtifactError::Io(
            "inactive transaction owner marker".into(),
        ))
    }
}
...
fn parse_owner(value: &str) -> Result<(i32, &str), ArtifactError> {
    let owner = value
        .strip_prefix(".transaction-owner-")
        .ok_or_else(|| ArtifactError::Io("unknown transaction control file".into()))?;
    let (pid, start) = owner
        .split_once('-')
        .ok_or_else(|| ArtifactError::Io("malformed transaction owner marker".into()))?;
    if pid.is_empty()
        || start.is_empty()
        ...
        || !start.bytes().all(|byte| byte.is_ascii_digit())
    {
        return Err(ArtifactError::Io(
            "malformed transaction owner marker".into(),
        ));
    }
    Ok((
        pid.parse::<i32>()
...
fn validate_marker_file(directory: &OwnedFd, name: &std::ffi::OsStr) -> Result<(), ArtifactError> {
    ...
    if metadata.st_mode & 0o170777 != 0o100600
...
fn scrub_stale_owners(directory: &OwnedFd) -> Result<(), ArtifactError> {
    ...
    for entry in fs::read_dir(path).map_err(io_error)? {
        ...
        let Ok((pid, start)) = parse_owner(&value) else {
            continue;
        };
        if validate_marker_file(directory, &name).is_err() {
            continue;
        }
...
fn errno(error: rustix::io::Errno) -> ArtifactError {
    ArtifactError::Io(error.to_string())
}

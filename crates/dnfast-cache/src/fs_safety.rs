...
use crate::model::{CacheError, FileRecord, io_error, sha256};
...
pub(crate) fn create_private_tree(root: &Path, target: &Path) -> Result<(), CacheError> {
    ...
    let relative = target
        .strip_prefix(root)
        .map_err(|_| CacheError::Corrupt("cache path escaped root".into()))?;
    let mut current = root.to_path_buf();
    for component in relative.components() {
        let std::path::Component::Normal(component) = component else {
            return Err(CacheError::Corrupt("unsafe cache path component".into()));
        };
...
fn create_private_root(root: &Path) -> Result<(), CacheError> {
        ...
        .is_err_and(|error| error.kind() == std::io::ErrorKind::NotFound)
    {
        ...
        current = current
            .parent()
            .ok_or_else(|| CacheError::Corrupt("cache root has no existing ancestor".into()))?;
    }
...
pub(crate) fn read_regular(path: &Path) -> Result<Vec<u8>, CacheError> {
    let parent = path
        .parent()
        .ok_or_else(|| CacheError::Corrupt("file has no parent".into()))?;
    let name = path
        .file_name()
        .ok_or_else(|| CacheError::Corrupt("file has no name".into()))?;
...
fn read_owned_retained(fd: OwnedFd, maximum: u64) -> Result<(Vec<u8>, File), CacheError> {
        ...
        || before.st_mode & 0o022 != 0
    {
        return Err(CacheError::Corrupt(
            "unsafe cache file ownership, mode, or links".into(),
        ));
    }
    let limit = maximum
        .checked_add(1)
        .ok_or_else(|| CacheError::Corrupt("file size overflow".into()))?;
    let capacity = usize::try_from(before.st_size)
        .map_err(|_| CacheError::Corrupt("file too large for platform".into()))?;
    if before.st_size < 0 || before.st_size as u64 > maximum {
...
pub(crate) fn validate_name(name: &str) -> Result<(), CacheError> {
    if name.is_empty()
        || name.contains('/')
        || name.contains('\\')
        || matches!(name, "." | ".." | "manifest.json")
    {
        return Err(CacheError::Corrupt(
            "unsafe or reserved manifest path".into(),
        ));
    }
    Ok(())
}

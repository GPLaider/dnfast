...
fn object(root: &std::path::Path, digest: &str) -> PathBuf { root.join("objects/sha256").join(digest) }
...
fn opens_each_complete_generation_by_repomd_digest() {
    ...
    // When
    let first = cache.publish_complete("main", &build9.0, &build9.1, &build9.2).expect("build9");
    let second = cache.publish_complete("main", &build10.0, &build10.1, &build10.2).expect("build10");

    // Then
    assert_ne!(first.digest, second.digest);
    assert_eq!(cache.open_by_digest(&first.digest).expect("old object").digest, first.digest);
    assert_eq!(cache.load("main").expect("current search").digest, second.digest);
...
fn same_generation_reuses_canonical_persisted_origin_on_reload() {
    ...
    let first = cache.publish_complete_with_origin("main", &build.0, &build.1, &build.2, Some("https://one.example/fedora/repodata/repomd.xml")).unwrap();
    let second = cache.publish_complete_with_origin("main", &build.0, &build.1, &build.2, Some("https://two.example/fedora/repodata/repomd.xml")).unwrap();
    let reloaded = Cache::new(directory.path()).open_by_digest(&first.digest).unwrap();
...
fn complete_publication_rejects_mixed_generations_without_changing_current() {
    ...
    let build9 = metadata("generated-build9");
    let build10 = metadata("generated-build10");
    let current = cache.publish_complete("main", &build9.0, &build9.1, &build9.2).expect("build9").digest;
    ...
    assert_eq!(cache.load("main").expect("previous current").digest, current);
}
...
fn legacy_manifest_requires_explicit_refresh() {
    ...
    fs::write(object.join("manifest.json"), br#"{"repomd":{},"primary":{},"packages":{}}"#).expect("legacy manifest");
...
fn open_by_digest_rejects_symlinked_metadata() {
    ...
    let cache = Cache::new(directory.path());
    let build10 = metadata("generated-build10");
    let snapshot = cache.publish_complete("main", &build10.0, &build10.1, &build10.2).expect("build10");
    let object = directory.path().join("objects/sha256").join(&snapshot.digest);
...
fn interrupted_staging_is_unreachable_and_preserves_current() {
    ...
    assert!(matches!(cache.open_by_digest(&"b".repeat(64)), Err(CacheError::Io(_))));
}
...
fn concurrent_competing_generations_never_publish_partial_state() {
    ...
    let handles = (0..4).map(|index| {
        ...
        std::thread::spawn(move || Cache::new(root).publish_complete("main", &generation.0, &generation.1, &generation.2))
    }).collect::<Vec<_>>();
    let results = handles.into_iter().map(|handle| handle.join().expect("publisher thread").expect("publication").digest).collect::<Vec<_>>();

    // Then
    assert_eq!(results.iter().collect::<std::collections::BTreeSet<_>>().len(), 2);
    let cache = Cache::new(&root);
    let current = cache.load("main").expect("current");
    assert!(results.contains(&current.digest));
    for digest in results { cache.open_by_digest(&digest).expect("complete competing object"); }
}
...
fn every_complete_metadata_file_is_revalidated_on_open() {
    ...
    let build10 = metadata("generated-build10");
    let snapshot = cache.publish_complete("main", &build10.0, &build10.1, &build10.2).expect("build10");
    let primary = directory.path().join("objects/sha256").join(&snapshot.digest).join("primary");
...
fn version_one_unknown_duplicate_and_extra_manifest_shapes_reject() {
    ...
    fs::write(&manifest, original.replacen("\"version\":2", "\"version\":1", 1)).expect("v1");
    assert!(matches!(cache.open_by_digest(&snapshot.digest), Err(CacheError::CacheUpgradeRequired)));
    fs::write(&manifest, original.replacen("{", "{\"unknown\":true,", 1)).expect("unknown");
    assert!(matches!(cache.open_by_digest(&snapshot.digest), Err(CacheError::Corrupt(_))));
    fs::write(&manifest, original.replacen("{", "{\"version\":2,", 1)).expect("duplicate");
    assert!(matches!(cache.open_by_digest(&snapshot.digest), Err(CacheError::Corrupt(_))));
    fs::write(&manifest, &original).expect("restore");
    fs::write(object(directory.path(), &snapshot.digest).join("extra"), b"extra").expect("extra");
    assert!(matches!(cache.open_by_digest(&snapshot.digest), Err(CacheError::Corrupt(_))));
}
...
fn forged_search_index_and_updated_manifest_hash_reject() {
    ...
    let mut value: serde_json::Value = serde_json::from_slice(&fs::read(&manifest).expect("manifest")).expect("json");
...
fn manifest_larger_than_eight_mib_rejects_before_deserialization() {
    ...
    let build10 = metadata("generated-build10");
    let snapshot = cache.publish_complete("main", &build10.0, &build10.1, &build10.2).expect("build10");
    fs::write(object(directory.path(), &snapshot.digest).join("manifest.json"), vec![b' '; 8 * 1024 * 1024 + 1]).expect("oversize manifest");
    assert!(matches!(cache.open_by_digest(&snapshot.digest), Err(CacheError::Corrupt(_))));
}
...
fn rename_faults_preserve_current_and_clean_staging() {
    ...
    let build9 = metadata("generated-build9");
    let build10 = metadata("generated-build10");
    let old = cache.publish_complete("main", &build9.0, &build9.1, &build9.2).expect("old").digest;
    fs::write(directory.path().join(".fail-before-object-rename"), b"1").expect("fault marker");
    assert!(cache.publish_complete("main", &build10.0, &build10.1, &build10.2).is_err());
    ...
    assert!(!directory.path().join("objects/sha256").read_dir().expect("objects").any(|entry| entry.expect("entry").file_name().to_string_lossy().starts_with(".staging-")));
...
fn same_repomd_different_repository_race_has_one_truthful_winner() {
    ...
    let handles = ["one", "two"].map(|repository| {
        ...
        std::thread::spawn(move || {
            barrier.wait();
            Cache::new(root).publish_complete(repository, &generation.0, &generation.1, &generation.2)
        })
    });
    let results = handles.map(|handle| handle.join().expect("publisher"));
    assert_eq!(results.iter().filter(|result| result.is_ok()).count(), 1);
    assert_eq!(results.iter().filter(|result| matches!(result, Err(CacheError::Corrupt(_)))).count(), 1);
    let cache = Cache::new(&root);
...
fn same_repomd_different_integrity_race_has_one_truthful_winner() {
    ...
    let barrier = std::sync::Arc::new(std::sync::Barrier::new(2));
    let complete = {
        let root = root.clone(); let generation = generation.clone(); let barrier = std::sync::Arc::clone(&barrier);
        std::thread::spawn(move || { barrier.wait(); Cache::new(root).publish_complete("main", &generation.0, &generation.1, &generation.2).map(|snapshot| snapshot.integrity) })
    };
    let search = {
        let root = root.clone(); let generation = generation.clone(); let barrier = std::sync::Arc::clone(&barrier);
        std::thread::spawn(move || { barrier.wait(); Cache::new(root).publish("main", &generation.0, &generation.1).map(|_| SnapshotIntegrity::SearchOnly) })
    };
    let results = [complete.join().expect("complete"), search.join().expect("search")];
    assert_eq!(results.iter().filter(|result| result.is_ok()).count(), 1);
    assert_eq!(results.iter().filter(|result| matches!(result, Err(CacheError::Corrupt(_)))).count(), 1);
    let cache = Cache::new(&root);
    let current = cache.load("main").expect("winner pointer");
    let opened = cache.open_by_digest(&current.digest).expect("winner object");
    assert_eq!(opened.integrity, *results.iter().find_map(|result| result.as_ref().ok()).expect("winner integrity"));
}

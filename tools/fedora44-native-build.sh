...
build_guest() {
  ...
  sources='native/src/common.c native/src/solver.c native/src/solver_state.c native/src/decisions.c native/src/actions.c native/src/installed.c native/src/inventory.c native/src/inventory_write.c native/src/transaction.c native/src/transaction_run.c native/src/transaction_result.c native/src/transaction_payload_fault.c native/src/keyring.c native/src/keyring_identity.c native/src/rpm_signature.c native/src/rpm_payload.c native/src/limits.c native/src/metadata_io.c native/src/rpm.c native/src/callbacks.c native/src/authority.c'
  packages='gcc-16.1.1-2.fc44.aarch64 libsolv-devel-0.7.39-1.fc44.aarch64 rpm-devel-6.0.1-2.fc44.aarch64 pkgconf-pkg-config-2.5.1-1.fc44.aarch64 rust-1.96.1-1.fc44.aarch64 cargo-1.96.1-1.fc44.aarch64 createrepo_c-1.2.1-5.fc44.aarch64'
  ...
  if ((PROBE)); then
    guest "export DNFAST_NATIVE_REAL=1; cd /home/fedora/src && gcc -std=c17 -Wall -Wextra -Werror -Inative/include native/tests/header_contract.c -o /tmp/header-contract && /tmp/header-contract && gcc $cflags -Inative/include -Inative/src \$(pkg-config --cflags libsolv rpm) $sources native/tests/native_probe.c \$(pkg-config --libs libsolv rpm) -lsolvext -ldl -lpthread -o /tmp/native-probe && /tmp/native-probe abi1 && /tmp/native-probe happy && /tmp/native-probe interrupt && /tmp/native-probe misleading && /tmp/native-probe malformed && cargo run --offline --locked -p dnfast-native --example probe -- happy && cargo run --offline --locked -p dnfast-native --example probe -- interrupt && cargo run --offline --locked -p dnfast-native --example probe -- panic"
    ...
    if ((SANITIZE)); then
      guest "export DNFAST_NATIVE_REAL=1; cd /home/fedora/src && printf '%s\\n' native_requested_spec_null_contract_lane=asan-ubsan && gcc $cflags -Inative/include -Inative/src \$(pkg-config --cflags libsolv rpm) $sources native/tests/requested_spec_null_contract.c \$(pkg-config --libs libsolv rpm) -lsolvext -ldl -lpthread -o /tmp/requested-spec-null-contract && ASAN_OPTIONS=verify_asan_link_order=0:detect_leaks=1:halt_on_error=1 UBSAN_OPTIONS=halt_on_error=1:print_stacktrace=1 /tmp/requested-spec-null-contract"
      ...
      guest "export DNFAST_NATIVE_REAL=1; cd /home/fedora/src && printf '%s\\n' native_requested_spec_null_contract_lane=normal && gcc $cflags -Inative/include -Inative/src \$(pkg-config --cflags libsolv rpm) $sources native/tests/requested_spec_null_contract.c \$(pkg-config --libs libsolv rpm) -lsolvext -ldl -lpthread -o /tmp/requested-spec-null-contract && /tmp/requested-spec-null-contract"

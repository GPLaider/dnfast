...
ok "probes every installed program for native linkage" contains "$HARNESS" 'do ldd \"\$path\" | grep -F '\''libsolv.so.1'\''; ldd \"\$path\" | grep -F '\''librpm.so.10'\''; ldd \"\$path\" | grep -F '\''libmodulemd.so.2'\''; done'
ok "pins the aarch64 modulemd build dependency" contains "$HARNESS" 'libmodulemd-devel-2.15.3-1.fc44.aarch64'
